...
export function isAShareRegularTradingMinute(time) {
  const { hour, minute } = parseClock(time);
...
export function isAShareClosingAuctionMinute(time) {
  const { hour, minute } = parseClock(time);
  if (!Number.isFinite(hour) || !Number.isFinite(minute) || hour > 23 || minute > 59) return false;
  const absoluteMinute = hour * 60 + minute;
  return absoluteMinute >= 14 * 60 + 57 && absoluteMinute <= 15 * 60;
}

export function isAShareContinuousTradingMinute(time) {
  return isAShareRegularTradingMinute(time) && !isAShareClosingAuctionMinute(time);
}

export function isAShareAfterHoursFixedPriceMinute(time) {
  const { hour, minute } = parseClock(time);
  if (!Number.isFinite(hour) || !Number.isFinite(minute) || hour > 23 || minute > 59) return false;
  const absoluteMinute = hour * 60 + minute;
  return absoluteMinute >= 15 * 60 + 5 && absoluteMinute <= 15 * 60 + 30;
}

// Backwards-compatible name: this is the regular intraday session used by Smart-T.
export const isAShareTradingMinute = isAShareRegularTradingMinute;
