...
export function normalizeStockPosition(raw, code) {
  ...
  if (raw.tradeShares != null) {
    normalized.tradeShares = Math.min(
      normalized.openingShares,
      Math.floor(nonNegativeInteger(raw.tradeShares) / 100) * 100,
    );
  }
  if (raw.maxDailyTrades != null) {
    normalized.maxDailyTrades = Math.max(
      1,
      Math.min(10, nonNegativeInteger(raw.maxDailyTrades) || 1),
    );
  }
