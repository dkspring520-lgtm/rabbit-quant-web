...
// Provider-neutral minute OHLC audit. It is deliberately pure so live and
// replay paths can apply the same guard before drawing candles or scoring.
export function auditMinuteOhlc(points = [], { maxRangePct = 0.04 } = {}) {
  const rows = Array.isArray(points) ? points : [];
  let valid = 0;
  let rejected = 0;
  const issues = [];
  for (const point of rows) {
    const values = [point?.open, point?.high, point?.low, point?.price ?? point?.close].map(Number);
    const [open, high, low, close] = values;
    if (![open, high, low, close].every(Number.isFinite) || [open, high, low, close].some(value => value <= 0)) {
      rejected += 1; issues.push({ time: String(point?.time ?? ""), reason: "缺少有效OHLC" }); continue;
    }
    const rangePct = (high - low) / close;
    const ordered = high >= Math.max(open, close) && low <= Math.min(open, close);
    if (!ordered || rangePct < 0 || rangePct > maxRangePct) {
      rejected += 1; issues.push({ time: String(point?.time ?? ""), reason: !ordered ? "高低价与开收盘不一致" : `单分钟振幅 ${(rangePct * 100).toFixed(2)}% 超限` }); continue;
    }
    valid += 1;
  }
  return { total: rows.length, valid, rejected, rejectionRate: rows.length ? rejected / rows.length : 0, issues };
}
...
export function marketTimestampMillis(value) {
  const text = String(value ?? "").trim();
  if (!text) return Number.NaN;
  const exchange = text.match(/^(\d{4})(\d{2})(\d{2})-(\d{2})(\d{2})(\d{2})(\d{3})$/);
  const normalized = exchange
    ? `${exchange[1]}-${exchange[2]}-${exchange[3]}T${exchange[4]}:${exchange[5]}:${exchange[6]}.${exchange[7]}+08:00`
    : text;
  return Date.parse(normalized);
}

export function shouldPreferL2Quote({
  exchangeTimestamp,
  quoteTimestamp = null,
  now = Date.now(),
  maximumAgeMs = 10_000,
  maximumLagBehindQuoteMs = 2_000,
} = {}) {
  const exchangeMillis = marketTimestampMillis(exchangeTimestamp);
  const nowMillis = now instanceof Date ? now.getTime() : Number(now);
  if (!Number.isFinite(exchangeMillis) || !Number.isFinite(nowMillis)) return false;
  if (exchangeMillis > nowMillis + 30_000 || nowMillis - exchangeMillis > maximumAgeMs) return false;

  const quoteMillis = marketTimestampMillis(quoteTimestamp);
  return !Number.isFinite(quoteMillis) || exchangeMillis + maximumLagBehindQuoteMs >= quoteMillis;
}
