...
import { hasFormalAlertScore } from './alert-delivery-policy.mjs';
export function studioSignals(alerts, code, date) {
  ...
  for (const alert of Array.isArray(alerts) ? alerts : []) {
    ...
    if (!data) continue;
    if (alert.level === 'formal' && !hasFormalAlertScore(data)) continue;
    ...
    if (!side) continue;
    const rawScore = alert.level === 'formal' ? data.confirmationScore ?? data.meta?.confirmationScore : data.score;
    const score = typeof rawScore === 'number' && Number.isFinite(rawScore) && rawScore >= 0 && rawScore <= 100 ? Math.round(rawScore) : null;
