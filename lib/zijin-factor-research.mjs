...
import {
  crossedVwapCausally,
  detectTrendContinuationConflict,
} from "./smart-t-engine.mjs";
...
export function calculateZijinTrendContinuationRisk(minutes, directionLabel) {
  const points = sanitizeMinutes(minutes);
  const direction = directionLabel === "反T" ? "SELL_FIRST" : "BUY_FIRST";
  if (points.length < 2) {
    return {
      blocked: false,
      reason: null,
  ...
  let totalVolume = 0;
  let priceSum = 0;
  const vwaps = points.map((point, index) => {
    const weight = Math.max(0, Number(point.volume) || 0);
    weightedPrice += point.price * weight;
    totalVolume += weight;
    priceSum += point.price;
    return totalVolume > 0 ? weightedPrice / totalVolume : priceSum / (index + 1);
  });
  const index = points.length - 1;
  const currentVwap = vwaps[index];
  const currentDeviation = percent(points[index].price, currentVwap);
  const vwapMomentum15 = index >= 15 ? percent(currentVwap, vwaps[index - 15]) : 0;
  const vwapMomentum30 = index >= 30 ? percent(currentVwap, vwaps[index - 30]) : 0;
  const priceMomentum60 = index >= 60 ? percent(points[index].price, points[index - 60].price) : 0;
  const priceMomentum90 = index >= 90 ? percent(points[index].price, points[index - 90].price) : 0;
  const broadPriceWindow = points.slice(Math.max(0, index - 89), index + 1);
  const broadPricePoints = broadPriceWindow.length;
  const longPriceMeanBias = percent(
    points[index].price,
    average(broadPriceWindow.map((point) => point.price)),
  );
  const sessionMove = percent(points[index].price, points[0].price);
  ...
  const pivotPrice = direction === "SELL_FIRST"
    ? Math.max(...pivotWindow.map((point) => point.price))
    : Math.min(...pivotWindow.map((point) => point.price));
  const pivotOffset = pivotWindow.findIndex((point) => point.price === pivotPrice);
  const pivotIndex = pivotWindowStart + Math.max(0, pivotOffset);
  const pivotAge = index - pivotIndex;
  const prePivotMove10 = percent(
    pivotPrice,
    points[Math.max(0, pivotIndex - 10)].price,
  );
  const pivotDeviation = percent(pivotPrice, vwaps[pivotIndex] ?? currentVwap);
  const crossedVwap = crossedVwapCausally({
    direction,
    pivotDeviation,
    currentDeviation,
  });
  const risk = detectTrendContinuationConflict({
    direction,
    currentDeviation,
    crossedVwap,
    vwapMomentum15,
  ...
  return {
    ...risk,
    direction,
    currentDeviation: round(currentDeviation),
    crossedVwap,
    vwapMomentum15: round(vwapMomentum15),
    vwapMomentum30: round(vwapMomentum30),
    sessionMove: round(sessionMove),
    prePivotMove10: round(prePivotMove10),
    pivotAge,
    priceMomentum60: round(priceMomentum60),
    priceMomentum90: round(priceMomentum90),
    longPriceMeanBias: round(longPriceMeanBias),
    broadPricePoints,
  };
}
...
function factorSnapshot(minutes, previousClose = null) {
  ...
  const trendContinuationRisk = calculateZijinTrendContinuationRisk(points, directionLabel);
  ...
  const label = status === "candidate"
    ...
    : trendContinuationRisk.blocked
      ? `${directionLabel}硬门禁：${trendContinuationRisk.reason}`
    : status === "watch"
