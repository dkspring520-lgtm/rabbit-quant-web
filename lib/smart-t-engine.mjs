...
export function qualifiesMatureSellReversalRiskOverride({
  ...
  const strongL2Confirmation = !orderFlow?.available
    || (
      orderFlow.pass === true
      && orderFlow.score >= Math.max(1, orderFlow.required ?? 3)
    );
  return direction === "SELL_FIRST"
    ...
    // confirmed. If L2 exists, it must additionally confirm the sell side.
    // A one-way warning may still exist, but a one-minute fade can never use
    // the exception because the maturity gate below remains mandatory.
    && scoreConfirmed
    && structuralConfirmation
...
export function confirmsEarlyOpeningRiskWithL2({
  opening = false,
  time = "",
  direction,
  pivotAge = 0,
  ...
  const riskyEarlyReverseT = opening
    && time < "0945"
  ...
  if (!riskyEarlyReverseT) return true;
  return orderFlow?.available === true
    && orderFlow.pass === true
    && orderFlow.score >= 2;
}
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        // so this cannot fade an accelerating rally. When L2 is present it
        // must give a unanimous 3/3 sell-side confirmation; missing L2 stays
        // neutral for historical and non-L2 replays.
        ...
        const openingRiskL2Confirmed = !(profile.requireEarlyOpeningRiskL2 > 0)
          || confirmsEarlyOpeningRiskWithL2({
            opening,
            time: point.time,
            direction,
            pivotAge,
            minPivotAge: profile.minEarlyOpeningRiskPivotAge ?? 4,
            trendContinuationBlocked: trendContinuationRisk.blocked,
            trendRiskGroups: trendRiskAssessment.groups,
            orderFlow,
          });
        ...
        if (!openingRiskL2Confirmed) openingRiskL2Blocked += 1;
        ...
        const executionGates = {
          ...
          openingRiskL2: openingRiskL2Confirmed,
        ...
        if (nowMinute - lastObservationMinute >= 8
          ...
          observations.push({
            ...
            edge,
            requiredEdge,
            l2Available: orderFlow.available,
            l2Pass: orderFlow.available ? orderFlow.pass : null,
            executable,
        ...
        if (executable) {
          ...
          const entryReason = `${trigger}；信号评分 ${score}/${profile.score}，预估空间 ${edge.toFixed(2)}%，成本门槛 ${requiredEdge.toFixed(2)}%，量比 ${ratio.toFixed(2)}${sizingReason}${flipReason}${riskOverrideReason}${orderFlow.available ? `；${orderFlow.reason}` : ""}`;
          fees += firstFee;
