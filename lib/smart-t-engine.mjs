...
export function resolveReplayPositionSize(plannedQuantity, mode = "fixed", evidence = {}) {
  ...
  if (planned < 100 || mode !== "quality-tiered") return planned;
  ...
  const fraction = score >= threshold + 2 && ratio >= 1 && structural && momentum
    ? 0.50
    : score >= threshold + 1 && structural
      ? 0.25
      : 0.10;
  return Math.max(100, Math.min(planned, roundLot(planned * fraction)));
}
 ...
 * @param {{capital:number,baseShares:number,sellable:number,feeRate:number,slippage:number,minCommission:boolean,slippageMode:"percent"|"tick",forceCloseTime:string,profile?:string,profileOverrides?:Record<string,number>,positionSizeMode?:"fixed"|"quality-tiered",minimumNetProfitAmount?:number,minimumGrossSpreadAmount?:number,previousClose?:number|null,randomValue?:number,strategyVersion?:string,gateAudit?:boolean,volatilityMode?:"fixed"|"causal-realized"|"causal-hybrid"}} options
 */
