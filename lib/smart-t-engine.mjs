...
const PROFILES = {
  ...
  "平衡档": { score: 4, cooldown: 8, candidateNetPct: 0.42, targetNetPct: 0.69, maxCycles: 1, deviation: 0.70, reversal: 0.22, maxSellPullback: 0.36, minBuyVolumeRatio: 0.80, minSellVolumeRatio: 0.90, minMomentum3: 0.12, minRewardRisk: 1.50, hardStopPct: 0.75, softStopPct: 0.40, softStopMinutes: 16, timeExitMinutes: 32, trailActivationPct: 0.30, trailRetracePct: 0.08, trailMinNetPct: 0.01, maxOpeningChasePct: 0.90, strongBuySessionMove: 0.60, strongBuyVwap30: 0.30, strongSellSessionMove: 0.90, strongSellVwap30: 0.30 },
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && cycleNets.length < profile.maxCycles && consecutiveLosses < 2 && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        const executable = !openingSignal?.candidateOnly && !openingChaseConflict && quantity >= 100 && score >= profile.score && structuralConfirmation && executionMomentumConfirmed && !regimeConflict && edge >= requiredEdge && rewardRisk >= profile.minRewardRisk && entryTimingValid && orderFlow.pass && cycleState.phase === "READY" && (!opening || openingUsed < 2);
    ...
    if (position) {
      ...
      const economicTakeProfit = projectedNetPct >= profile.targetNetPct;
      ...
      const trailActivationPct = profile.trailActivationPct ?? Number.POSITIVE_INFINITY;
      const trailRetracePct = profile.trailRetracePct ?? 0.18;
      const trailMinNetPct = profile.trailMinNetPct ?? 0.03;
      const trailingProfit = !takeProfit
        && bestMove >= trailActivationPct
        && bestMove - move >= trailRetracePct
        && projectedNetPct >= trailMinNetPct;
        ...
        const exitReason = forceExit
          ...
          : stop
            ...
            : takeProfit
              ...
              : trailingProfit
                ? `浮盈回撤保护：已观察最大有利波动 ${bestMove.toFixed(2)}%，回撤 ${(bestMove - move).toFixed(2)}% 后锁定预计净收益 ${projectedNetPct.toFixed(2)}%`
