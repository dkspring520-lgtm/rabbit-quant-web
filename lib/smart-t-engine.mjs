...
export function detectFallingKnifeConflict({
  ...
  const reason = latePersistentDecline
...
export function detectRisingKnifeConflict({
  ...
  const blocked = broadVwapRise
    ...
    || earlyPersistentRise
    || latePersistentRise;
  const reason = latePersistentRise
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        const executable = executionQuotaAvailable && !openingSignal?.candidateOnly && !openingChaseConflict && !counterTrendQualityConflict && !recentTrendContinuationRisk && !shadowConflict && quantity >= 100 && score >= profile.score && structuralConfirmation && executionMomentumConfirmed && directionFlipConfirmed && !regimeConflict && edge >= requiredEdge && rewardRisk >= profile.minRewardRisk && entryTimingValid && orderFlow.pass && cycleState.phase === "READY" && (!opening || openingUsed < 2);
