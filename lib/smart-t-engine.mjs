...
function causalRangeEvidence(points, index, vwaps) {
  const end = index - 8;
  if (end < 35) return { confirmed: false, crossings: 0, amplitude: 0, vwapDrift: 0 };
  const start = Math.max(0, end - 45);
  const biases = [];
  const prices = [];
  ...
  for (const bias of biases) {
    const side = bias >= 0.10 ? 1 : bias <= -0.10 ? -1 : 0;
    if (!side) continue;
  ...
  const visitedBothSides = Math.min(...biases) <= -0.25 && Math.max(...biases) >= 0.25;
  return {
    confirmed: crossings >= 2 && visitedBothSides && amplitude >= 1.0 && vwapDrift <= 0.35,
    crossings,
    amplitude,
    vwapDrift,
  };
    ...
    if (!position && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        const rangeEvidence = v41Shadow
          ? causalRangeEvidence(points, index, vwaps)
        ...
        const shadowDirectionConflict = v41Shadow && !shadowDirectionAllowed;
        const shadowLunchRestartConflict = v41Shadow && lunchRestartWindow;
        const shadowEntryWindowConflict = v41Shadow && (point.time < "0945" || point.time > "1110");
        const shadowLocationThreshold = Math.max(0.35, profile.deviation * 0.65);
        const shadowLocationConflict = v41Shadow && (direction === "BUY_FIRST"
          ? deviation > -shadowLocationThreshold
          : deviation < shadowLocationThreshold);
        const shadowParticipationConflict = v41Shadow && ratio < 0.90;
        ...
        // V4 keeps its established execution behavior. V4.1 shadow promotes
        // the broad cycle from an explanation to a hard shadow-order gate, so
        // the experiment cannot silently change production V4 decisions.
        if (cycleConflict) cycleConflicts += 1;
        const regimeConflict = ((v41Shadow && cycleConflict) || (rawRegimeConflict && !localRegimeBreak) || strongSessionConflict)
        ...
        const executable = executionQuotaAvailable && !openingSignal?.candidateOnly && !openingChaseConflict && !counterTrendQualityConflict && !shadowConflict && quantity >= 100 && score >= profile.score && structuralConfirmation && executionMomentumConfirmed && !regimeConflict && edge >= requiredEdge && rewardRisk >= profile.minRewardRisk && entryTimingValid && orderFlow.pass && cycleState.phase === "READY" && (!opening || openingUsed < 2);
...
export { PROFILES, buildCandidateObservationCycles, causalCyclePreference, causalRangeEvidence, minutesFromOpen };
