...
function causalFormalEntryWindowConflict(time, direction, opening = false, profile = BASE_PROFILE) {
  ...
  if (!(profile.precisionEntryWindows > 0)) return time >= "1300";
      ...
      if (direction) {
        ...
        if (nowMinute - lastObservationMinute >= 8
          ...
          observations.push({
            ...
            edge,
            requiredEdge,
            orderFlowShadowAvailable: orderFlowShadow.available,
            orderFlowShadowAligned: orderFlowShadow.available ? orderFlowShadow.pass : null,
            executable,
        ...
        if (executable) {
          ...
          const orderFlowShadowNote = orderFlowShadow.available
            ? `；订单流影子观察：${orderFlowShadow.reason}（不影响正式条件）`
            : "";
          const entryReason = `${trigger}；信号评分 ${score}/${profile.score}，预估空间 ${edge.toFixed(2)}%，成本门槛 ${requiredEdge.toFixed(2)}%，量比 ${ratio.toFixed(2)}${sizingReason}${flipReason}${riskOverrideReason}${orderFlowShadowNote}`;
          fees += firstFee;
          ...
          actions.push({
            ...
            meta: {
              ...
              orderFlowShadow: {
                available: orderFlowShadow.available,
                aligned: orderFlowShadow.pass,
                score: orderFlowShadow.score,
                required: orderFlowShadow.required,
                reason: orderFlowShadow.reason,
              },
