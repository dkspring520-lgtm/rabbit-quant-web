 ...
 * @param {{capital:number,baseShares:number,sellable:number,feeRate:number,slippage:number,minCommission:boolean,slippageMode:"percent"|"tick",forceCloseTime:string,profile?:string,profileOverrides?:Record<string,number>,minimumNetProfitAmount?:number,minimumGrossSpreadAmount?:number,previousClose?:number|null,randomValue?:number,strategyVersion?:string,gateAudit?:boolean,volatilityMode?:"fixed"|"causal-realized"|"causal-hybrid"}} options
 */
      ...
      if (direction) {
        ...
        if (executable) {
          ...
          const riskOverrideReason = matureSellReversalRiskOverride
            ...
            : "";
          const entryReason = `${trigger}；信号评分 ${score}/${profile.score}，预估空间 ${edge.toFixed(2)}%，成本门槛 ${requiredEdge.toFixed(2)}%，量比 ${ratio.toFixed(2)}${flipReason}${riskOverrideReason}${orderFlow.available ? `；${orderFlow.reason}` : ""}`;
          fees += firstFee;
