...
const PROFILES = {
  "稳健档": { score: 6, cooldown: 10, minNetPct: 0.55, maxCycles: 1, deviation: 0.90, reversal: 0.32, maxSellPullback: 0.42 },
  "平衡档": { score: 4, cooldown: 8, minNetPct: 0.42, maxCycles: 1, deviation: 0.70, reversal: 0.22, maxSellPullback: 0.36 },
  "灵敏档": { score: 5, cooldown: 5, minNetPct: 0.32, maxCycles: 2, deviation: 0.65, reversal: 0.22, maxSellPullback: 0.32 },
  "量化学习": { score: 6, cooldown: 8, minNetPct: 0.42, maxCycles: 1, deviation: 0.78, reversal: 0.26, maxSellPullback: 0.38 },
};
...
export function runSmartTReplay(minutes, options) {
  ...
  if (points.length < 6 || normalQuantity < 100) return emptyResult(options.capital, "真实分时样本或可卖底仓不足，未生成交易");
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && cycleNets.length < profile.maxCycles && consecutiveLosses < 2 && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        const structuralConfirmation = opening || (
          ...
          && ratio >= (direction === "BUY_FIRST" ? 0.80 : 0.90)
          && ratio < 3
          && (direction === "BUY_FIRST" ? localMomentum3 >= 0.12 : localMomentum3 <= -0.12)
        );
        ...
        const strongSessionConflict = shortTrendConflict || persistentSessionConflict;
        ...
        const executable = !openingSignal?.candidateOnly && quantity >= 100 && score >= profile.score && structuralConfirmation && !regimeConflict && edge >= requiredEdge && rewardRisk >= 1.5 && entryTimingValid && (!opening || openingUsed < 2);
        if (nowMinute - lastObservationMinute >= 8 && observations.length < 3) {
          ...
          if (!entryTimingValid) blockers.push(`已离峰值回落 ${pivotReversal.toFixed(2)}%，不在低位追卖`);
        ...
        if (executable) {
          ...
          const trigger = openingSignal
            ...
            : direction === "BUY_FIRST"
              ...
              : `价格偏离 VWAP ${Math.abs(deviation).toFixed(2)}% 后回落 ${faded.toFixed(2)}%，距已观察峰值 ${pivotReversal.toFixed(2)}%`;
    ...
    if (position) {
      ...
      // A VWAP-loss exit is valid only after price first reached the favourable
      // side of VWAP.  Without this state check, a buy opened below VWAP could
      // be sold as soon as it became barely profitable even though no VWAP
      // support had actually been lost.
      const vwapProfitExit = profitArmed && position.favorableVwapSeen && move >= profitableFloor && (
          ...
          : stop
            ...
            : takeProfit
              ? trailingTakeProfit
                ...
                : `VWAP 利润保护：价格曾到达均价有利侧后重新失守，当前仍保留 ${move.toFixed(2)}% 浮盈`
