...
export function runSmartTReplay(minutes, options) {
      ...
      if (direction) {
        ...
        if (quantity >= 100 && score >= profile.score && structuralConfirmation && !regimeConflict && edge >= requiredEdge && rewardRisk >= 1.5 && (!opening || openingUsed < 2)) {
          ...
          const trigger = openingSignal
            ? `${direction === "BUY_FIRST" ? "低开转强" : "高开转弱"}；开盘价与 VWAP 方向确认`
            : direction === "BUY_FIRST"
      ...
      if (takeProfit || stop || timeExit || forceExit) {
        ...
        const exitReason = forceExit
          ...
          : stop
            ...
            : takeProfit
              ? `止盈闭环：本循环浮动 ${move.toFixed(2)}%，最佳浮动 ${bestMove.toFixed(2)}%，已满足 VWAP/回撤条件`
  ...
  return { net: cash - options.capital, gross, fees, executionCost, maxDrawdown, trades: cycleNets.length, wins, days: 1, curve, curveTimes, cycleNets, startTime: points[revealStart].time, status, actions, diagnostics: { candidates, cashBlocked, costBlocked, regimeBlocked, openingRegimeOverrides, consecutiveLosses } };
}
