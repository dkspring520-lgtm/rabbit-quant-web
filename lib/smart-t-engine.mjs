...
// when the entry pivot is intact and at least two causal structure votes still
// support the trade direction; otherwise the review closes the cycle.  A hard
...
export function evaluateAdaptiveTimeExit({
  ...
  const extended = pivotIntact && supportVotes >= 2;
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        // Use the narrow veto while the day still has no valid cycle. Once a
        // properly confirmed cycle has completed, the ordinary grouped risk
        // budget resumes control; repeatedly applying this audit veto to later
        // oscillations needlessly destroys closure coverage.
        const obviousDirectionalEntryError = cycleNets.length === 0
