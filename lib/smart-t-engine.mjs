...
function confirmCandidateDirectionFlip({
  ...
  return pairEconomicallyDistinct
    && nowMinute - oppositeCandidate.minute >= cooldown
    && structuralConfirmation
    && executionMomentumConfirmed;
}
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        const oppositeCandidate = lastQualifiedObservation?.direction !== direction
          ? lastQualifiedObservation
          : null;
        ...
        const executable = executionQuotaAvailable && !openingSignal?.candidateOnly && !openingChaseConflict && !counterTrendQualityConflict && !shadowConflict && quantity >= 100 && score >= profile.score && structuralConfirmation && executionMomentumConfirmed && directionFlipConfirmed && !regimeConflict && edge >= requiredEdge && rewardRisk >= profile.minRewardRisk && entryTimingValid && orderFlow.pass && cycleState.phase === "READY" && (!opening || openingUsed < 2);
        if (nowMinute - lastObservationMinute >= 8
          ...
          if (candidateQualified) lastQualifiedObservation = { direction, price: point.price, minute: nowMinute, time: point.time };
          observationCounts[session] += 1;
          lastObservationMinute = nowMinute;
        }
