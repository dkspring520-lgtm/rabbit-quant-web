  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        // Trend protection is intentionally a vote instead of a chain of
        // overlapping one-vote vetoes. The detectors share inputs (VWAP
        // slopes, session move and broad price momentum), so treating every
        // one as an independent hard AND condition collapses the executable
        // set without adding equivalent information.
        const trendRiskVotes = [
          cycleConflict,
          persistentDirectionConflict && !localRegimeBreak,
          broadTrendConflict,
          strongSessionConflict,
          ...
          trendContinuationRisk.blocked || recentTrendContinuationRisk,
        ].filter(Boolean).length;
