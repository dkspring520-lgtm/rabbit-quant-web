...
export function evaluateAdaptiveTimeExit({
  ...
  // The review clock must not crystallise a small, recoverable loss while the
  // entry pivot is still intact.  This is deliberately narrower than an
  // unconditional pivot hold: once the trade is executable at break-even or
  // better, the normal structure votes decide whether to keep it, so a weak
  // position cannot occupy the T inventory until the maximum hold time.
  ...
  const extended = pivotIntact && (lossProtected || supportVotes >= requiredSupportVotes);
...
function addCoverageOnlyObservations(points, observations) {
  ...
  for (const definition of definitions) {
    ...
    const pivot = visiblePoints.reduce((selected, point) => {
      if (definition.pivot === "low") return point.price < selected.price ? point : selected;
      return point.price > selected.price ? point : selected;
    }, visiblePoints[0]);
