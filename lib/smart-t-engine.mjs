 ...
 * @param {{capital:number,baseShares:number,sellable:number,feeRate:number,slippage:number,minCommission:boolean,slippageMode:"percent"|"tick",forceCloseTime:string,profile?:string,profileOverrides?:Record<string,number>,minimumNetProfitAmount?:number,minimumGrossSpreadAmount?:number,previousClose?:number|null,randomValue?:number}} options
 */
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        // Keep the broader cycle as an explainable preference instead of a
        // universal execution veto. A/B testing on the full Zijin minute
        // history showed that a hard or soft cycle gate reduced out-of-sample
        // quality. Existing causal trend/structure gates still decide orders.
        if (cycleConflict) cycleConflicts += 1;
        const regimeConflict = ((rawRegimeConflict && !localRegimeBreak) || strongSessionConflict)
        ...
        const executable = executionQuotaAvailable && !openingSignal?.candidateOnly && !openingChaseConflict && !counterTrendQualityConflict && quantity >= 100 && score >= profile.score && structuralConfirmation && executionMomentumConfirmed && !regimeConflict && edge >= requiredEdge && rewardRisk >= profile.minRewardRisk && entryTimingValid && orderFlow.pass && cycleState.phase === "READY" && (!opening || openingUsed < 2);
