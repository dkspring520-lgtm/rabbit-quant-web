...
// Keep the chart readable without letting the first morning swing consume the
// whole day's observation budget. A stock may surface up to four VWAP
// displacement alerts (one high/low per session) plus later confirmations.
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && cycleNets.length < profile.maxCycles && consecutiveLosses < 2 && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        if (nowMinute - lastObservationMinute >= 8 && observations.length < MAX_DAILY_OBSERVATIONS) {
          const blockers = [];
