...
const PROFILES = {
  ...
  "稳健档": { score: 6, cooldown: 10, candidateNetPct: 0.55, targetNetPct: 0.74, maxCycles: 1, deviation: 0.90, reversal: 0.32, maxSellPullback: 0.42 },
  "平衡档": { score: 4, cooldown: 8, candidateNetPct: 0.42, targetNetPct: 0.69, maxCycles: 1, deviation: 0.70, reversal: 0.22, maxSellPullback: 0.36 },
  "灵敏档": { score: 5, cooldown: 5, candidateNetPct: 0.32, targetNetPct: 0.64, maxCycles: 2, deviation: 0.65, reversal: 0.22, maxSellPullback: 0.32 },
  "量化学习": { score: 6, cooldown: 8, candidateNetPct: 0.42, targetNetPct: 0.69, maxCycles: 1, deviation: 0.78, reversal: 0.26, maxSellPullback: 0.38 },
};
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && cycleNets.length < profile.maxCycles && consecutiveLosses < 2 && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        const requiredEdge = candidateRequiredEdge;
