...
function openingDirection(points, index, previousClose, vwap) {
  if (!previousClose || index < 8) return null;
  ...
  const two = points.slice(index - 1, index + 1);
  const fiveMinuteBase = points[Math.max(0, index - 5)].price;
  const followThrough = Math.abs(pct(current, fiveMinuteBase));
  if (followThrough < 0.45) return null;
  if (gap <= -0.30 && two.every((point) => point.price > open && point.price > vwap) && current > previous) return "BUY_FIRST";
  if (gap >= 0.30 && two.every((point) => point.price < vwap) && current < open && current < previous) return "SELL_FIRST";
  return null;
}
...
function emptyResult(capital, status, diagnostics = {}) {
  return { net: 0, gross: 0, fees: 0, executionCost: 0, maxDrawdown: 0, trades: 0, wins: 0, days: 0, curve: [capital], curveTimes: [], cycleNets: [], startTime: "", status, actions: [], diagnostics };
}
...
export function runSmartTReplay(minutes, options) {
  ...
  const boundedRandom = Math.min(0.999999, Math.max(0, options.randomValue ?? 0));
  const revealStart = Math.min(points.length - 30, Math.max(10, 10 + Math.floor(boundedRandom * Math.max(1, points.length * 0.62))));
  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && cycleNets.length < profile.maxCycles && consecutiveLosses < 2 && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      let direction = opening ? openingDirection(points, index, options.previousClose, vwap) : null;
      ...
      if (direction) {
        ...
        const regimeConflict = (direction === "BUY_FIRST" && regime === "downtrend") || (direction === "SELL_FIRST" && regime === "uptrend");
        ...
        const quantity = opening ? openingQuantity : normalQuantity;
        const approximateCosts = ((orderCosts("买入", point.price, quantity, options) + orderCosts("卖出", point.price, quantity, options)) / (point.price * quantity) * 100) + (options.slippageMode === "tick" ? options.slippage / point.price * 200 : options.slippage * 2);
        ...
        if (quantity >= 100 && score >= profile.score && structuralConfirmation && !regimeConflict && edge >= requiredEdge && rewardRisk >= 1.5 && (!opening || openingUsed < 2)) {
          ...
          position = { direction, rawEntry: point.price, entry: executed, quantity, entryTime: point.time, entryIndex: index, firstFee, cycleId: cycleNets.length + 1, opening };
          actions.push({ time: point.time, side: firstSide, price: executed, quantity, curveIndex: curve.length, direction: direction === "BUY_FIRST" ? "正T" : "反T", cycleId: position.cycleId });
          bestMove = 0;
    ...
    if (position) {
      ...
      const nearVwap = position.direction === "BUY_FIRST" ? point.price >= vwap * 0.998 : point.price <= vwap * 1.002;
      const takeProfit = (hold >= 12 && nearVwap && move >= profitableFloor) || (hold >= 25 && move >= Math.max(0.60, profitableFloor)) || (bestMove >= 0.55 && bestMove - move >= 0.30 && move >= profitableFloor);
      const stop = move <= -0.85 || (hold >= (position.opening ? 10 : 8) && move <= -0.48);
      const timeExit = hold >= 32;
      ...
      if (takeProfit || stop || timeExit || forceExit) {
        ...
        actions.push({ time: point.time, side: secondSide, price: executed, quantity: position.quantity, curveIndex: curve.length, direction: position.direction === "BUY_FIRST" ? "正T" : "反T", cycleId: position.cycleId });
        position = null;
  ...
  const status = cycleNets.length
    ...
    : `Smart-T V4 本次未形成可执行闭环：候选 ${candidates}，趋势拦截 ${regimeBlocked}，成本/盈亏比拦截 ${costBlocked}。`;
  return { net: cash - options.capital, gross, fees, executionCost, maxDrawdown, trades: cycleNets.length, wins, days: 1, curve, curveTimes, cycleNets, startTime: points[revealStart].time, status, actions, diagnostics: { candidates, costBlocked, regimeBlocked, consecutiveLosses } };
}
