  ...
  for (let index = revealStart; index < points.length; index += 1) {
    ...
    if (!position && cycleNets.length < profile.maxCycles && consecutiveLosses < 2 && liquidEntryWindow && nowMinute - lastExitMinute >= profile.cooldown) {
      ...
      if (direction) {
        ...
        const executable = !openingSignal?.candidateOnly && quantity >= 100 && score >= profile.score && structuralConfirmation && !regimeConflict && edge >= requiredEdge && rewardRisk >= profile.minRewardRisk && entryTimingValid && orderFlow.pass && cycleState.phase === "READY" && (!opening || openingUsed < 2);
