...
/**
 * Keep the live cockpit and the single-day replay on the same observation
 * stream.  Both sides may contribute engine observations and strict L2 repair
 * observations; the later value wins for the same causal key, matching the
 * live monitor's historical-event semantics.
 */
export function mergeZijinReplayObservations(observations = [], repairObservations = []) {
  const merged = new Map();
  for (const observation of [...(observations ?? []), ...(repairObservations ?? [])]) {
  ...
  return [...merged.values()].sort((left, right) => String(left.time).localeCompare(String(right.time)));
}
