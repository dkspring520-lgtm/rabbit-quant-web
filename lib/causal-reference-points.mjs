  ...
  for (let index = 6; index < points.length; index += 1) {
    ...
    if (pivot.time !== current.time && reversal >= 0.10 && lastStep >= 0.015) {
      turns.push({
        ...
        pivotPrice,
        pivotLabel: direction === "正T" ? "此前低位" : "此前高位",
        pivotAssessment: "unconfirmed",
        confirmationLabel: direction === "正T" ? "反弹确认" : "回落确认",
...
function rangeReference(points, direction) {
  ...
  return {
    ...
    pivotAssessment: "unconfirmed",
    confirmationLabel: direction === "正T" ? "前低确认" : "前高确认",
    blockers: ["尚未形成有效反转", "自动补充参考，不是候补买卖点或下单指令"],
    reason: direction === "正T"
      ? `截至 ${current.time} 才能确认此前低位 ${pivotPrice.toFixed(2)}；这是观察记录，不是当前买点`
      : `截至 ${current.time} 才能确认此前高位 ${pivotPrice.toFixed(2)}；这是观察记录，不是当前卖点`,
  };
}
