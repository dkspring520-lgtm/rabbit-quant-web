  ...
  for (let index = 6; index < points.length; index += 1) {
    ...
    if (pivot.time !== current.time && reversal >= 0.10 && lastStep >= 0.015) {
      turns.push({
        ...
        confirmationLabel: direction === "正T" ? "候补买点" : "候补卖点",
        blockers: ["仅为当时可确认的转折参考", "候补观察层，不是下单指令"],
        reason: direction === "正T"
...
export function buildCausalReferencePoints(minutes, observations = []) {
  ...
  return ["正T", "反T"].flatMap((direction) => {
    ...
    const selected = [];
    for (const item of [...rankedExisting, ...generated]) {
      if (selected.length >= 2) break;
      if (selected.some((entry) => entry.time === item.time)) continue;
      selected.push(item);
    }
