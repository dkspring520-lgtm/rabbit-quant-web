...
export function scoreGrade(score) {
  if (!validScore(score)) return "待评分";
  score = Math.round(score);
  if (score < 20) return "极差";
  if (score < 40) return "很差";
...
export function buyRiskPresentation(observation, strategy = "closure") {
  const score = observationConfirmationScore(observation, strategy);
  return `买入风险提示 · 买入确认 ${signalStrengthPresentation({ score }).label}`;
}
...
export function signalStrengthPresentation({ score = null, historicalProbability = null } = {}) {
  ...
  if (validScore(score)) return {
    label: `${Math.round(score)}分 · ${scoreGrade(score)}`,
