...
export const ZIJIN_OPENING_PLAYBOOK = Object.freeze({
  ...
  candidateScore: 75,
  lateLocationGuardStart: "1015",
  maximumLateVwapOvershootPct: 0.30,
  extremeRangePct: 4.5,
...
function recentVolumeRatio(points) {
  ...
  const recentCount = points.length < 6 ? 2 : 3;
  const recent = points.slice(-recentCount).map((point) => point.volume).filter((value) => value > 0);
  const baseline = points.slice(Math.max(0, points.length - 23), -recentCount)
...
export function evaluateZijinOpeningPlaybook(minutePrefix, options = {}) {
  ...
  if (points.length < ZIJIN_OPENING_PLAYBOOK.minimumPoints) {
    return baseResult("waiting", asOfTime, points.length, [
      `09:30 已开始实时扫描；当前仅观察到 ${asOfTime}，至少积累 ${ZIJIN_OPENING_PLAYBOOK.minimumPoints} 个真实分钟点后才显示候选方向。`,
      "不使用之后出现的高点、低点或收盘结果。",
    ]);
  }
  ...
  const lateLocationPass = asOfTime < ZIJIN_OPENING_PLAYBOOK.lateLocationGuardStart
    || (direction === "正T"
      ? distanceToVwapPct <= ZIJIN_OPENING_PLAYBOOK.maximumLateVwapOvershootPct
      : distanceToVwapPct >= -ZIJIN_OPENING_PLAYBOOK.maximumLateVwapOvershootPct);
  ...
  if (!lateLocationPass) {
    reasons.push(direction === "正T"
      ? "10:15后价格已明显高于VWAP，正T候选降为观察，避免追高。"
      : "10:15后价格已明显低于VWAP，反T候选降为观察，避免低位补卖。");
  }
