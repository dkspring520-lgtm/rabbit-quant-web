...
function attachZijinPreopenPermission(evaluation, preopenGate) {
  ...
  const effectivePermission = preopenGate?.status === "blocked"
    && String(evaluation.asOfTime || "") >= "0935"
    ? { ...permission, active: true, wouldBlock: true, reason: "盘前方向未获09:35多源确认，候选保持中性观察" }
    : permission;
  const blockedCandidate = evaluation.status === "candidate"
    && ((effectivePermission.active && effectivePermission.wouldBlock)
      || (preopenGate?.status === "blocked" && String(evaluation.asOfTime || "") >= "0935"));
  return {
    ...evaluation,
    status: blockedCandidate ? "watch" : evaluation.status,
    title: blockedCandidate ? `${evaluation.title} · 方向许可观察` : evaluation.title,
    reasons: effectivePermission.active ? [...evaluation.reasons, effectivePermission.reason] : evaluation.reasons,
    metrics: { ...evaluation.metrics, preopenPermission: effectivePermission },
  };
}
