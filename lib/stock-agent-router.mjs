...
export function evaluateStockAgent({ code, minutes = [], previousClose = null } = {}) {
  ...
  if (latestTime <= "1030") {
    ...
    const trendContinuationRisk = opening.direction
      ...
      : null;
    const hardBlocked = opening.status === "candidate" && trendContinuationRisk?.blocked;
    return {
      agent,
      phase: "opening",
      status: hardBlocked ? "watch" : opening.status,
      direction: opening.direction,
      score: opening.score,
      asOfTime: opening.asOfTime,
      title: hardBlocked
        ? `${opening.direction}硬门禁`
      ...
      reasons: hardBlocked
        ? [
          `硬门禁：${trendContinuationRisk.reason}。`,
          "下跌延续不先买、上涨延续不先卖；等待实时反转确认后再重新评估。",
          ...opening.reasons,
        ]
        : [...opening.reasons, orderFlow.reason],
      metrics: {
