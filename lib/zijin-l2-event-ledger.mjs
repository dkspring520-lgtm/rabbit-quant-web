...
export function buildMatureZijinL2Labels({
  ...
  for (const observation of observations) {
    ...
    const costModel = costPct == null
      ? calculateZijinEconomicThreshold(anchorPrice, costOptions)
      : {
          id: "legacy-fixed-threshold",
          mode: "fixed",
          requiredGrossMovePct: Math.max(0, finite(costPct)),
          roundTripCostPct: Math.max(0, finite(costPct)),
          minimumNetEdgePct: 0,
          minimumGrossSpreadPct: 0,
        };
    const requiredGrossMovePct = costModel.requiredGrossMovePct;
    for (const horizon of horizons) {
      const labelId = `${observation.observationId}:${horizon}m:${costModel.id}`;
      ...
      labels.push({
        ...
        favorablePct,
        adversePct,
        reachedCost: favorablePct >= requiredGrossMovePct,
        netDirectionalWin: favorablePct - requiredGrossMovePct > adversePct,
        costPct: requiredGrossMovePct,
        costModel,
        causal: true,
      });
    }
