...
export function buildDailyReviewChart({ minutes = [], observations = [], trades = [], simulatedActions = [] } = {}) {
  ...
  const markerInputs = [
    ...observations.filter((item) => /卖飞|T飞|冲高回落|下跌未止|下降途中|跌破 VWAP|VWAP 下方/i.test(`${item?.reason ?? ""} ${(item?.blockers ?? []).join(" ")}`)).map((item, index) => ({
      id: `risk-${index}-${normalizeTime(item?.time)}`,
      time: item?.time, price: finitePositive(item?.price),
      kind: /下跌未止|下降途中|跌破 VWAP|VWAP 下方/i.test(`${item?.reason ?? ""} ${(item?.blockers ?? []).join(" ")}`) ? "buy-risk" : "t-fly-risk",
      label: (() => { const score = Number(item?.score); const grade = score >= 0 && score <= 100 ? (score < 20 ? "极差" : score < 40 ? "很差" : score < 60 ? "偏弱" : score < 70 ? "及格" : score < 80 ? "OK" : "超好") : null; return /下跌未止|下降途中|跌破 VWAP|VWAP 下方/i.test(`${item?.reason ?? ""} ${(item?.blockers ?? []).join(" ")}`) ? `买入风险${grade ? ` · ${score}分 · ${grade}` : "：先别接"}` : "T飞风险：先别卖"; })(),
