...
export function buildDailyReviewChart({ minutes = [], observations = [], trades = [], simulatedActions = [] } = {}) {
  ...
  const markerInputs = [
    ...observations.filter((item) => /卖飞|T飞|冲高回落|下跌未止|下降途中|跌破 VWAP|VWAP 下方/i.test(`${item?.reason ?? ""} ${(item?.blockers ?? []).join(" ")}`)).map((item, index) => ({
      ...
      label: (() => { const score = Number(item?.score); const tier = score >= 80 ? "确认买点" : score >= 70 ? "谨慎买点" : "观察买点"; return /下跌未止|下降途中|跌破 VWAP|VWAP 下方/i.test(`${item?.reason ?? ""} ${(item?.blockers ?? []).join(" ")}`) ? `买入风险${Number.isFinite(score) ? ` · ${tier} ${score}分` : "：先别接"}` : "T飞风险：先别卖"; })(),
