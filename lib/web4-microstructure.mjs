...
function buildBehavior(points, liveL2, cvd) {
  const flow = normalizeQmtOrderFlow(liveL2 ?? points.at(-1) ?? {});
  const buySweep = finite(flow.buySweepStreak);
  const sellSweep = finite(flow.sellSweepStreak);
  const transactionCount = finite(flow.transactionCount);
  const priorCounts = (points ?? []).slice(-20, -1)
    .map(point => finite(normalizeQmtOrderFlow(point).transactionCount))
    .filter(value => value !== null && value > 0);
  const baselineCount = median(priorCounts);
  const intensity = transactionCount !== null && baselineCount ? transactionCount / baselineCount : null;
  const impactEfficiency = cvd.available && cvd.recent.length >= 3
    ? Math.abs((cvd.recent.at(-1).price - cvd.recent[0].price) / Math.max(cvd.recent[0].price, .01)) /
      Math.max(cvd.recent.reduce((sum, row) => sum + Math.abs(row.gross), 0), 1e-9)
    : null;
  const bidDepth = finite(flow.bid1Volume);
  const askDepth = finite(flow.ask1Volume);
  const priorDepths = (points ?? []).slice(-20, -1).map(point => {
    const row = normalizeQmtOrderFlow(point);
    return (row.bid1Volume ?? 0) + (row.ask1Volume ?? 0);
  }).filter(value => value > 0);
  const depthBaseline = median(priorDepths);
  const liquidityVacuum = bidDepth !== null && askDepth !== null && depthBaseline !== null
    ? bidDepth + askDepth < depthBaseline * .55 : null;
  const icebergBuy = finite(flow.icebergBuyStrength ?? flow.buyIcebergStrength);
  const icebergSell = finite(flow.icebergSellStrength ?? flow.sellIcebergStrength);
  return {
    sweeps: { buy: buySweep, sell: sellSweep, available: buySweep !== null || sellSweep !== null },
    iceberg: { buy: icebergBuy, sell: icebergSell, available: icebergBuy !== null || icebergSell !== null },
    intensity: { value: intensity, available: intensity !== null },
    impactEfficiency: { value: impactEfficiency, available: impactEfficiency !== null },
    liquidityVacuum: { value: liquidityVacuum, available: liquidityVacuum !== null },
  };
}
...
export function evaluateWeb4Microstructure({
  ...
  return {
    ...
    buyScore, sellScore, buyGroups, sellGroups, rvol, cvd, absorption, book, behavior, evidence,
  };
}
