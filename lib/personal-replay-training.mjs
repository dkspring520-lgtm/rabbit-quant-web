...
export function summarizePersonalTraining({ initialCash, initialShares, initialPrice, cash, shares, markPrice, actions = [] }) {
  ...
  const cycles = summarizeTrainingCycles(actions);
  return {
    initialEquity,
    equity,
    net: equity - initialEquity,
    fees,
    actionCount: actions.length,
    tradeGross: cycles.gross,
    tradeFees: cycles.fees,
    tradeNet: cycles.net,
    closedQuantity: cycles.closedQuantity,
    closedCycles: cycles.closedCycles,
    openQuantity: cycles.openQuantity,
    openSide: cycles.openSide,
  };
}

export function summarizeTrainingCycles(actions = []) {
  const queue = [];
  let gross = 0;
  let fees = 0;
  ...
  for (const rawAction of Array.isArray(actions) ? actions : []) {
    const side = rawAction?.side === "sell" ? "sell" : "buy";
    const quantity = Math.max(0, Math.floor(finite(rawAction?.quantity)));
    const price = Math.max(0, finite(rawAction?.executionPrice, rawAction?.marketPrice));
    const feePerShare = quantity > 0 ? Math.max(0, finite(rawAction?.fee)) / quantity : 0;
    ...
    while (remaining > 0 && queue.length && queue[0].side !== side) {
      ...
      const buyPrice = side === "buy" ? price : opening.price;
      const sellPrice = side === "sell" ? price : opening.price;
      gross += (sellPrice - buyPrice) * matched;
      fees += (feePerShare + opening.feePerShare) * matched;
    ...
    if (remaining > 0) queue.push({ side, quantity: remaining, price, feePerShare });
  }

  const openQuantity = queue.reduce((total, item) => total + item.quantity, 0);
  return {
    gross,
    fees,
    net: gross - fees,
    ...
    closedCycles,
    openQuantity,
    openSide: openQuantity ? queue[0]?.side ?? null : null,
  };
}
