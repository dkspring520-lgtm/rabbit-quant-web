...
export function buildZijinPricePlan({
  ...
  const positiveRiskUnit = Math.max(projectedMove * 0.68, reference * 0.0024, 0.04);
  const positiveStop = roundPrice(Math.max(0.01, Math.min(buyLow - 0.01, support - positiveRiskUnit * 0.55)));
  ...
  const reverseStop = roundPrice(Math.max(sellHigh + 0.01, resistance + positiveRiskUnit * 0.55));
