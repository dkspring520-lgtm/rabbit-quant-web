...
export function compactChartLabelKeys(observations, maxLabels = 3) {
  ...
  const selected = [];
  // A completed-day phone chart must visibly change after lunch. Previously
  // the three highest scores could all come from the morning, so afternoon
  // causal dots existed but every afternoon text label stayed hidden. Reserve
  // one label for each populated session before balancing buy/sell direction.
  for (const session of ["morning", "afternoon"]) {
    const best = ranked.find((entry) => (
      session === "morning" ? entry.item.time < "1300" : entry.item.time >= "1300"
    ));
    if (best && selected.length < maxLabels && !selected.includes(best)) selected.push(best);
  }
