...
export function evaluateZijinFundResponse(bars = [], windowSize = 5) {
  ...
  const persistence = usable.filter(row => Math.sign(finite(row.netNotional)) === direction).length / usable.length;
  ...
  } else if (netNotional < 0 && aligned && priceChangePercent <= -.08) {
    state = "outflow";
    label = "主动流出";
    message = "主动净卖与价格同向，注意回落";
  } else if (netNotional < 0) {
    state = "waiting";
    label = "卖压待确认";
