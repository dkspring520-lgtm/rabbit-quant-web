...
/**
 * Causal five-minute price/fund response classifier for the shadow layer.
 * It reads only completed L2 minute rows available at the evaluation time and
 * never supplies a formal strategy gate.
 */
export function evaluateZijinFundResponse(bars = [], windowSize = 5) {
  ...
  const bearishFlowShadow = outflowAligned;
  ...
  if (netNotional > 0 && aligned && priceChangePercent >= .1) {
    ...
    message = outflowDecelerating
      ? "主动净卖仍与价格同向，但卖压正在放缓；仅影子观察，不影响正式正T"
      : "主动净卖与价格同向；订单流影子提示：注意回落，不影响正式正T";
  } else if (netNotional < 0) {
    state = "waiting";
    label = "卖压待确认";
