...
export function closeTCycle(state, order) {
  ...
  const holdMinutes = order.minute - state.pending.entryMinute;
  const minHoldMinutes = Math.max(0, Number(order.minHoldMinutes) || 0);
  if (!order.forced && holdMinutes < minHoldMinutes) {
    return { ok: false, state, reason: `持仓仅 ${holdMinutes} 分钟，尚未达到 ${minHoldMinutes} 分钟最短确认时间` };
  }
