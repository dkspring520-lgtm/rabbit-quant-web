...
export function upgradeShadowState(value, now = new Date().toISOString()) {
  ...
  const upgraded = { ...value, schemaVersion: 3, updatedAt: value.updatedAt || now };
...
export function processVisibleMinute(state, context) {
  const { marketDate, minutes, index, previousClose, peers = [], externalContext = null, preopenGate = null } = context;
  ...
  const events = resolveDirectionPermissionOutcomes(state, { minutes, index });
  ...
  for (const [modelKey, config] of Object.entries(SHADOW_MODELS)) {
    ...
    if (decision.passed) {
      ...
      const directionPermissionSignal = recordDirectionPermissionCandidate(state, {
        marketDate,
        model: modelKey,
        point,
        index,
