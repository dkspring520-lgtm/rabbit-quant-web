...
export function computeVisibleFeatures({ minutes, index, previousClose, peers = [], externalContext = null }) {
  ...
  return {
    ...
    // External factors are prospective evidence only. They are intentionally
    // excluded from the frozen A-E candidate rules until a new hypothesis is
    // preregistered and validated out of sample.
    externalContext,
  };
}
...
export function processVisibleMinute(state, context) {
  ...
  const features = computeVisibleFeatures({ minutes, index, previousClose, peers, externalContext });
