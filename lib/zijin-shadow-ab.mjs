...
function buildLiveL2Snapshot(l2 = null) {
  ...
  const sellVotes = [
    activeBuyRatio !== null && activeBuyRatio <= 0.47,
  ...
  return {
    ...
    sellVotes,
    qualityBlocked,
    pass: available && !qualityBlocked && sellVotes >= 2,
  };
}

export function computeVisibleFeatures({ minutes, index, previousClose, peers = [], externalContext = null, marketDate = null, l2 = null }) {
...
export function upgradeShadowState(value, now = new Date().toISOString()) {
  ...
  const upgraded = { ...value, schemaVersion: 4, updatedAt: value.updatedAt || now };
...
export function processVisibleMinute(state, context) {
  const { marketDate, minutes, index, previousClose, peers = [], externalContext = null, preopenGate = null, l2 = null } = context;
  resetForDate(state, marketDate);
  const point = minutes[index];
  const features = computeVisibleFeatures({ minutes, index, previousClose, peers, externalContext, marketDate, l2 });
  ...
  for (const [modelKey, config] of Object.entries(SHADOW_MODELS)) {
    ...
    if (decision.passed) {
      ...
      const rangeReverseQualitySignal = modelKey === "H"
        ...
        : null;
      if (!observationOnly) model.today.activeTrade = { pendingEntry: true, candidateTime: point.time, features: candidateFeatures };
