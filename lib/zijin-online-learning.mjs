...
export const ZIJIN_ONLINE_LEARNING_VERSION = "2026.08.25-shadow";

export const ZIJIN_EXTERNAL_RESEARCH_HYPOTHESES = Object.freeze([
  Object.freeze({
    id: "douyin-houshanren-20260825-staged-exit",
    creator: "后山人csAJ",
    source: "抖音三级线索",
    sourceUrl: "https://www.douyin.com/note/7677778105591739366",
    publishedAt: "2026-08-25T09:40:18+08:00",
    title: "BOLL位置、MACD衰减与KDJ高位拐头的分阶段候卖确认",
    direction: "候卖/反T确认",
    timeframe: "1分钟、5分钟和日线分别验证",
    hypothesis: "价格触及或沿BOLL上轨运行后，MACD柱缩短或快线拐头，再由KDJ从80以上向下交叉确认，可能比单指标卖点更少误报。",
    invalidation: "强趋势沿上轨运行且量价、订单流继续增强时，不应仅因KDJ超买否决持有。",
    requiredData: ["BOLL", "MACD", "KDJ", "volume", "OFI"],
    validationStatus: "pending-backtest",
    affectsFormalStrategy: false,
    canTrade: false,
  }),
  Object.freeze({
    id: "douyin-ameng-20260824-triple-volume-retest",
    creator: "A梦先生",
    source: "抖音三级线索",
    sourceUrl: "https://www.douyin.com/video/7677556320882907813",
    publishedAt: "2026-08-24T19:19:39+08:00",
    title: "三倍量转强后缩量回踩10日线",
    direction: "正T日线背景",
    timeframe: "日线",
    hypothesis: "下跌趋势中出现显著放量并推动均线转强，随后缩量回踩10日线不破，可作为正T背景加分，而不是分钟级直接买点。",
    invalidation: "回踩放量跌破10日线、均线未转强或所谓三倍量缺少统一基准时，假设失效。",
    requiredData: ["daily-volume", "MA10", "moving-average-alignment"],
    validationStatus: "pending-definition-and-backtest",
    affectsFormalStrategy: false,
    canTrade: false,
  }),
  Object.freeze({
    id: "douyin-ameng-20260823-macd-top-divergence",
    creator: "A梦先生",
    source: "抖音三级线索",
    sourceUrl: "https://www.douyin.com/video/7677179256544321017",
    publishedAt: "2026-08-23T18:56:27+08:00",
    title: "日线MACD顶背离叠加量价转弱并由周线确认",
    direction: "反T/减仓背景",
    timeframe: "日线+周线",
    hypothesis: "价格创新高但MACD动能和成交量未同步增强，再获得周线转弱确认，可能提升高位反T或减仓背景判断。",
    invalidation: "价格创新高同时量能、MACD和周线趋势继续增强时，顶背离风险标签应取消。",
    requiredData: ["daily-MACD", "weekly-MACD", "daily-volume", "price-highs"],
    validationStatus: "pending-backtest",
    affectsFormalStrategy: false,
    canTrade: false,
  }),
...
export function createZijinOnlineLearningObservations({
  ...
  const eventCandidateRules = events.items.slice(0, 3).map(item => ({
  ...
  const researchCandidateRules = ZIJIN_EXTERNAL_RESEARCH_HYPOTHESES.map(item => ({
    id: item.id,
    title: item.title,
    direction: item.direction,
    source: `${item.source}·${item.creator}`,
    rule: item.hypothesis,
    validationStatus: item.validationStatus,
  }));
  const candidateRules = [...eventCandidateRules, ...researchCandidateRules];
  ...
  const evidenceBody = {
    ...
    researchHypotheses: ZIJIN_EXTERNAL_RESEARCH_HYPOTHESES.map(item => ({
      id: item.id,
  ...
  return {
    ...
    candidateRules,
    researchHypotheses: ZIJIN_EXTERNAL_RESEARCH_HYPOTHESES,
    onlineLearning: {
