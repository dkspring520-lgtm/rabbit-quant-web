...
export const FACTOR_ENGINE_VERSION = "1.1.0";
...
function normalizePoint(raw, previous = null) {
  ...
  return {
    ...
    marketOpenGap: finite(raw?.marketOpenGap),
    sectorOpenGap: finite(raw?.sectorOpenGap),
    goldOpenGap: finite(raw?.goldOpenGap),
    copperOpenGap: finite(raw?.copperOpenGap),
    ...
    bid1Price: flow.bid1Price,
    ask1Price: flow.ask1Price,
    nearTouchImbalance: flow.nearTouchImbalance,
    microprice: flow.microprice,
    micropriceEdgeBps: flow.micropriceEdgeBps,
  };
}
...
export function normalizeFactorSession(session) {
  ...
  return {
    ...
    marketOpenGap: finite(session?.marketOpenGap),
    sectorOpenGap: finite(session?.sectorOpenGap),
    goldOpenGap: finite(session?.goldOpenGap),
    copperOpenGap: finite(session?.copperOpenGap),
...
export class CausalFactorContext {
  constructor(session, asOfIndex, derived = null, sameMinuteHistory = null) {
    this.session = session;
    this.asOfIndex = asOfIndex;
    this.derived = derived ?? computeDerived(session.minutes);
    this.sameMinuteHistory = sameMinuteHistory;
  }
  ...
  historicalSameMinute(field) {
    const time = this.point()?.time;
    const values = this.sameMinuteHistory?.get(time)?.[field];
    return Array.isArray(values) ? values : [];
  }
}

function historicalZScore(current, history, minimumSamples = 10) {
  const values = history.filter(Number.isFinite);
  const deviation = stddev(values);
  return Number.isFinite(current) && values.length >= minimumSamples && deviation > 0
    ? (current - average(values)) / deviation
    : null;
}
...
function factorValue(factorId, context) {
  ...
  const valueMap = {
    ...
    "vwap.same_minute_zscore": () => historicalZScore(
      vwapBias === null ? null : vwapBias - 1,
      context.historicalSameMinute("vwapBias"),
    ),
    ...
    "volume.same_minute_zscore": () => historicalZScore(
      Math.log1p(point.volume),
      context.historicalSameMinute("logVolume"),
    ),
    ...
    "market.relative_strength_context": () => {
      const contextReturns = [context.session.marketOpenGap, context.session.sectorOpenGap].filter(Number.isFinite);
      return contextReturns.length ? point.price / points[0].open - 1 - average(contextReturns) : null;
    },
    "market.sector_resonance": () => context.session.sectorOpenGap === null
      ? null
      : average([point.price / points[0].open - 1, context.session.sectorOpenGap]),
    "market.metal_resonance": () => {
      const returns = [context.session.goldOpenGap, context.session.copperOpenGap].filter(Number.isFinite);
    ...
    "orderflow.ofi_persistence_5m": () => {
      const values = [];
      for (let cursor = index; cursor >= 0 && values.length < 5; cursor -= 1) {
        if (derived.ofi[cursor] === null) break;
        values.unshift(derived.ofi[cursor]);
      }
      if (values.length < 3) return null;
      const mean = average(values);
      const direction = Math.sign(mean);
      const aligned = direction === 0 ? 0 : values.filter(value => Math.sign(value) === direction).length / values.length;
      return mean * aligned;
    },
    ...
    "orderflow.microprice_edge": () => point.micropriceEdgeBps === null
      ? null
      : point.micropriceEdgeBps / 10_000,
...
export class FactorEngine {
  ...
  computeSession(rawSession, { factorIds = null, startIndex = 0, sameMinuteHistory = null } = {}) {
    ...
    for (let index = Math.max(0, startIndex); index < session.minutes.length; index += 1) {
      const context = new CausalFactorContext(session, index, derived, sameMinuteHistory);
      rows.push({
        date: session.date,
  ...
  computeSessions(sessions, options = {}) {
    ...
    const sameMinuteHistory = new Map();
    const output = [];
    for (const rawSession of ordered) {
      const computed = this.computeSession(rawSession, { ...options, sameMinuteHistory });
      output.push(computed);
      const derived = computeDerived(computed.session.minutes);
      for (let index = 0; index < computed.session.minutes.length; index += 1) {
        const point = computed.session.minutes[index];
        const bucket = sameMinuteHistory.get(point.time) ?? { vwapBias: [], logVolume: [] };
        const vwapRatio = safeRatio(point.price, derived.vwaps[index]);
        if (vwapRatio !== null) bucket.vwapBias.push(vwapRatio - 1);
        bucket.logVolume.push(Math.log1p(point.volume));
        if (bucket.vwapBias.length > 60) bucket.vwapBias.shift();
        if (bucket.logVolume.length > 60) bucket.logVolume.shift();
        sameMinuteHistory.set(point.time, bucket);
      }
    }
    return output;
  }
