...
function applyZijinV29L2Confirmation(decision, computed, rowByIndex) {
  ...
  return Object.freeze({
    ...decision,
    formal: Boolean(decision.formal && l2Available && !shallowGapDepthVeto && !midGapMicropriceVeto),
    l2Available,
    depthImbalance,
    activeBuyRatio,
    micropriceEdgeBps,
...
export function runZijinV29ShadowReplay(session, options = {}) {
  ...
  const baseDecisions = buildZuoTShadowDecisions([computed], { experimentId: "v2-standalone" });
  const decisions = baseDecisions.map(decision => applyZijinV29L2Confirmation(decision, computed, rowByIndex));
  return buildReplayResult({
    computed,
    decisions,
    config,
