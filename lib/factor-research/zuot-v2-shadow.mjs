...
function withinResearchWindow(time, config) {
  const ordinal = tradingMinuteOrdinal(time);
  return Number.isFinite(ordinal)
    && ordinal >= config.avoidOpeningMinutes
...
export function evaluateZuoTShadowRow({
  ...
  const inWindow = withinResearchWindow(row?.time, config);
...
export function runZuoTV1ReconstructedReplay(session, options = {}) {
  ...
  const observations = candidateEvents.map(event => {
    ...
    return {
      ...
      reason: `V1重建投票 ${decision.v1VoteCount}/7，VWAP、量能、MACD、OFI及技术位置共同确认。`,
      candidateKey: `zuot-v1-${decision.date}-${decision.direction}-${event.startOrdinal}`,
    };
  });
  ...
  for (const [index, trade] of trades.entries()) {
    ...
    actions.push({
      ...
      direction,
      cycleId,
      reason: `zuoT-v1 ${trade.direction === "positiveT" ? "正T低吸" : "反T高抛"} · 组合投票通过`,
  ...
  return {
    ...
    status: trades.length
      ? `zuoT-v1 重建影子完成：${trades.length} 个闭环，已扣除费用与双向滑点。`
      : `zuoT-v1 重建影子未形成闭环：${candidateEvents.length} 个候选事件，${formalSignals.length} 个正式信号。`,
    actions,
    observations,
    diagnostics: {
