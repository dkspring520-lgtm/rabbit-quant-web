    ...
    observationReason: decision => {
      ...
      const meanlineLabel = meanline?.confirmed
        ...
        : meanline?.state === "awaiting-orderflow"
          ? "均线/MACD出现，等待真实L2"
          : "均线/MACD/订单流未同时确认";
      return `V1重建投票 ${decision.v1VoteCount}/8；结构确认 ${decision.v1Structure.score}/4；${meanlineLabel}（影子）`;
    },
    observationDetails: decision => ({
  ...
  return buildReplayResult({
    ...
    observationReason: decision => {
      ...
      const structureNote = decision.direction === "reverseT"
        ...
        : "";
      return `V1情境影子：15分钟方向概率 ${probabilityLabel}；独立确认 ${decision.context.confirmationCount}/4${structureNote}。`;
    },
    observationDetails: decision => ({
