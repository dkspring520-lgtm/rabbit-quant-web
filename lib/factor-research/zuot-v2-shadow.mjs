...
export const ZUOT_V1_RECONSTRUCTED_VERSION = "1.1.0-houshanren-18";
...
function enrichV1Structure(computed, options = {}) {
  ...
  const enriched = rows.map((row, index) => {
    ...
    const reverseVolumeResponse = return1m !== null && Math.abs(return1m) <= config.maximumV1StallReturn1m
      && ((volumeRatio !== null && volumeRatio >= config.minimumV1StallVolumeRatio)
        || (volumeMomentum !== null && volumeMomentum > 0));
    ...
    const positiveVeto = Number.isFinite(ordinal)
...
export function evaluateZuoTShadowRow({
  ...
  if (candidate && experimentId === "v1-reconstructed-baseline" && v1Structure.available) {
    ...
    if (v1Structure.vetoed) rejectionReasons.push("v1-persistent-below-vwap");
  }
  return Object.freeze({
...
export function runZuoTV1ReconstructedReplay(session, options = {}) {
  ...
  const computed = enrichV1Structure(
    factorEngine.computeSession(session, { factorIds: ZUOT_V1_RECONSTRUCTED_FACTOR_IDS }),
    config,
  );
  ...
  return buildReplayResult({
    ...
    observationReason: decision => `V1重建投票 ${decision.v1VoteCount}/7；结构确认 ${decision.v1Structure.score}/4，需同时满足VWAP距离、MACD反转及背离或量价响应。`,
