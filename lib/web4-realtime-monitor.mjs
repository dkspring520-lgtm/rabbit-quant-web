 ...
 *
 * The fast path is deliberately deterministic and display-only. Technical
 * analysis may open a candidate, while L2 flow, A/H linkage and external/event
 * risk provide independent *shadow* evidence. This monitor must never promote,
 * block, or otherwise mutate the formal T execution state machine.
 */
export function evaluateWeb4RealtimeMonitor({
  ...
  const shadowAligned = status === "confirmed" && confidence >= 60;
  ...
  return {
    ...
    direction,
    confidence,
    // Deliberately explicit for every caller: WEB 4.0 and its L2/order-flow
    // inputs are research/diagnostic evidence only, never a formal gate.
    shadowAligned,
    affectsFormal: false,
    candidate,
    nonTechnicalSupport,
    ...
    summary: shadowAligned
      ? "技术、订单流与外部证据在影子层同向，仅供观察"
