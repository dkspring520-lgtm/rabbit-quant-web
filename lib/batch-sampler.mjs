...
/**
 * Build a randomized, de-duplicated queue while pushing recently selected
 * keys to the back. Consecutive UI batches stay visibly different whenever
 * the source universe is large enough.
 */
export function randomizedUniqueQueue(items, seed, recentKeys = [], key = "code") {
  if (!Array.isArray(items)) throw new TypeError("items must be an array");
  if (!Array.isArray(recentKeys)) throw new TypeError("recentKeys must be an array");
  ...
  const fresh = unique.filter((item) => !recent.has(String(item[key])));
  const repeated = unique.filter((item) => recent.has(String(item[key])));
  return [
    ...sampleWithSeed(fresh, fresh.length, `${seed}:fresh`),
    ...sampleWithSeed(repeated, repeated.length, `${seed}:recent`),
  ];
}
