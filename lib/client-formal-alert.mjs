import { hasFormalAlertScore, isRiskExitAction } from './alert-delivery-policy.mjs';
...
function normalizeMarketTime(value) {
  ...
  if (!/^\d{2}:?\d{2}$/.test(raw)) throw requestError("信号时间无效");
  const time = raw.replace(":", "");
  if (Number(time.slice(0, 2)) > 23 || Number(time.slice(2)) > 59) throw requestError("信号时间无效");
...
export function normalizeClientFormalAlert(input, { monitors = [], now = new Date() } = {}) {
  ...
  const confirmationScore = input?.action?.confirmationScore ?? input?.confirmationScore;
  const meta = input?.action?.meta;
  const riskExit = isRiskExitAction(input?.action);
  if (!riskExit && !hasFormalAlertScore({ confirmationScore })) throw requestError("正式信号确认评分必须为60-100分");
  ...
  const validPair = riskExit
    ? direction === '正T' ? side === '卖出' : side === '买入' || side === '买回'
    : direction === "正T" ? side === "买入" || side === "卖出" : side === "卖出" || side === "买入" || side === "买回";
  ...
  const eventKey = `${dateKey}:${code}:${riskExit ? 'risk-exit' : 'formal'}:client-v4:${direction}:${side}:${time}`;
  const actionLabel = direction === "反T" ? (side === "卖出" ? "先卖" : "买回") : (side === "卖出" ? "卖出" : "买入");

  return {
    code,
    level: riskExit ? "risk" : "formal",
    title: riskExit ? `${String(monitor.name ?? code).slice(0, 40)} · 风险退出提醒` : `${String(monitor.name ?? code).slice(0, 40)} · ${direction}${actionLabel} · 正式`,
    ...
    payload: {
      source: "client-v4",
      marketDate,
      action: { time, price, side, direction, reason, ...(riskExit ? { meta: { phase: 'exit', stop: meta.stop === true, timeExit: meta.timeExit === true, forceExit: meta.forceExit === true } } : { confirmationScore }) },
    },
  };
}
