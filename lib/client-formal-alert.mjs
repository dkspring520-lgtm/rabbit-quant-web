...
export function normalizeClientFormalAlert(input, { monitors = [], now = new Date() } = {}) {
  ...
  const confirmationScore = Number(input?.action?.confirmationScore ?? input?.confirmationScore);
  if (!Number.isFinite(confirmationScore) || confirmationScore < 60 || confirmationScore > 100) throw requestError("正式信号确认评分必须为60-100分");
