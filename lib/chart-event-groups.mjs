...
export function groupChartEvents(paths=[],observations=[],scores=[]) {
  ...
  return [...groups.values()].sort((a,b)=>a.time.localeCompare(b.time)||a.price-b.price).map(group=>{
    ...
    const shortContext=(context??'路径').split(/[·]/).at(-1).trim().replace(/观察$/,'');
    const shortLabel=scores.length?(conflict?'买卖分歧':scores.map(s=>`${s.label}${s.score}分`).join(' / ')):observation?`${shortContext} · ${observation.score}分`:`${shortContext} · 观察`;
