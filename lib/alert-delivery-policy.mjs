...
export function hasFormalAlertScore(action) {
  const score = action?.confirmationScore ?? action?.meta?.confirmationScore;
  return typeof score === "number" && Number.isFinite(score) && score >= 60 && score <= 100;
}

export function isRiskExitAction(action) {
  const meta = action?.meta;
  return meta?.phase === 'exit' && (meta.stop === true || meta.timeExit === true || meta.forceExit === true);
}
...
export function alertDirection(alert) {
  ...
  // A formal action's actual side is authoritative.  In particular, a
  // 反T cycle exits with a 买回 action; classifying by direction first would
  // announce that buyback as another sell signal.
  if (/卖/.test(String(action?.side || ""))) return "sell";
  if (/买/.test(String(action?.side || ""))) return "buy";
  if (String(action?.direction || "") === "反T") return "sell";
  if (String(action?.direction || "") === "正T") return "buy";
