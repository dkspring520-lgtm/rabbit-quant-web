...
for (const s of sessions) {
  ...
  if (!total) { total = lot(90000/ref); sellable = total; cash -= total*ref; initialShares = total; initialRef = ref; }
...
console.log(JSON.stringify({kind:'zijin-crossday-t1',year,sessions:daily.length,trades,rejected,costs:Number(costs.toFixed(2)),startEquity:first,endEquity:last,change:Number((last-first).toFixed(2)),holdEndEquity:Number(holdLast.toFixed(2)),excessVsHold:Number((last-holdLast).toFixed(2)),daily},null,2));
