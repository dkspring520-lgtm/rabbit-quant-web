...
console.log(JSON.stringify({kind:'zijin-crossday-t1',year,lateReverseCutoff,execution:{signalToNextMinute:true,slippagePct:0.02,t1:true,feesIncluded:true,holdBenchmark:true,untradableSkipped:true},sessions:daily.length,trades,rejected,rejectReasons,costs:Number(costs.toFixed(2)),cycles:completedCycles.length,incompleteCycle:openLeg?{direction:openLeg.direction,quantity:openLeg.quantity,entryPrice:openLeg.price}:null,wins:wins.length,losses:losses.length,winRatePct:completedCycles.length?Number((wins.length/completedCycles.length*100).toFixed(2)):0,profitFactor:grossLoss?Number((grossProfit/grossLoss).toFixed(3)):null,averageNet:completedCycles.length?Number((completedCycles.reduce((n,c)=>n+c.net,0)/completedCycles.length).toFixed(2)):0,startEquity:first,endEquity:last,change:Number((last-first).toFixed(2)),holdEndEquity:Number(holdLast.toFixed(2)),excessVsHold:Number((last-holdLast).toFixed(2)),maxDrawdown:Number(maxDrawdown.toFixed(2)),maxDrawdownPct:Number(maxDrawdownPct.toFixed(2)),daily},null,2));
