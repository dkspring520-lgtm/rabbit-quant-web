...
for (const s of sessions) {
  ...
  const replay = runSmartTReplay(rows, { capital: 200000, baseShares: initial, sellable: initial, feeRate: .025, slippage: .02, minCommission: true, slippageMode: "percent", forceCloseTime: "1450", previousClose: s.previousClose, profile: "平衡档", randomValue: 0 });
...
console.log(JSON.stringify({ kind: "zijin-t1-backtest", sessions: reports.length, totalRejected: sum("rejected"), totalCosts: Number(sum("costs").toFixed(2)), strategyValue: Number(sum("strategyValue").toFixed(2)), holdValue: Number(sum("holdValue").toFixed(2)), excessVsHold: Number(sum("excessVsHold").toFixed(2)), reports }, null, 2));
