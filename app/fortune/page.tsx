...
const horizonOptions=[
  {value:"5",label:"短线",detail:"5日"},
  {value:"20",label:"波段",detail:"20日"},
  {value:"60",label:"中期",detail:"60日"}
] as const;
...
function ziPingFor(date:string,score:number,momentum:number,rsi:number){
  const month=Number(date.split("-")[1])||1;
  const climate=month<=2||month>=11?"寒局取火":month>=5&&month<=8?"燥局取水":month<=4?"木旺宜疏":"金旺宜润";
  const pattern=score>=62?"顺势成格":score<=38?"逆势受制":"中和待变";
  const flow=Math.abs(momentum)<1?"气机未动":momentum>0?(rsi>68?"旺极防折":"生发流通"):(rsi<32?"衰极待复":"收敛下行");
  return {label:`${pattern} · ${climate}`,detail:flow};
}
...
export default function FortunePage(){
  ...
  useEffect(()=>{
    const timer=window.setTimeout(()=>setUiTheme(document.documentElement.dataset.theme==="light"?"light":"dark"),0);
  ...
  useEffect(()=>{
    ...
    const timer=window.setTimeout(()=>{if(sharedCode&&/^\d{6}$/.test(sharedCode))setCode(sharedCode)},0);
    return()=>window.clearTimeout(timer);
  },[]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    const timer=window.setTimeout(()=>{readingRun.current+=1;setReadingStatus("idle");setReadingProgress(0);setReadingStage("等待起卦")},0);
    return()=>window.clearTimeout(timer);
  ...
  const analysis=useMemo(()=>{
    ...
    let gains=0,losses=0;values.slice(-15).forEach((v,i,a)=>{if(i){const d=v-a[i-1];if(d>0)gains+=d;else losses-=d}});
  ...
  const projectedBars=useMemo(()=>{
    ...
    const totalMove=direction*(count<=5?.045:count<=20?.1:.18)*chanFactor*exhaustion;
     const forecastClose=(index:number)=>{
       ...
       return Math.max(.01,last*(1+totalMove*progress+wave+noise));
     };
     return Array.from({length:count},(_,index)=>{
       const previous=index?forecastClose(index-1):last;
       const open=previous*(1+(seeded(`${code}-${index}-open`)-.5)*.008);
       const close=forecastClose(index);
  ...
  const projectionBand=useMemo(()=>{
    const uncertainty=Math.max(.012,Math.min(.065,.018+Math.abs(analysis.score-50)/100*.045+(Number(horizon)>=60?.012:0)));
    return projectedBars.map((bar,index)=>{
      const widen=uncertainty*(.45+(index+1)/Math.max(1,projectedBars.length)*.55);
      return {high:bar.close*(1+widen),low:Math.max(.01,bar.close*(1-widen))};
    });
  },[analysis.score,horizon,projectedBars]);
  useEffect(()=>{
    ...
    const drawChart=()=>{
      ...
      const all=[...history,...projectedBars],bounds=[...history,...projectionBand],min=Math.min(...bounds.map(bar=>bar.low)),max=Math.max(...bounds.map(bar=>bar.high)),range=max-min||1;
  ...
  },[analysis.values,analysis.hasCenter,analysis.centerHigh,analysis.centerLow,marketBars,projectedBars,projectionBand,horizon]);
  const trend=analysis.score>58?"偏多":analysis.score<42?"偏空":"震荡";
  const actionGuide=trend==="偏多"?{do:"宜等回踩承接或上破确认后再观察",avoid:"忌在靠近上行确认位时盲目追高",focus:`关注 ¥${analysis.resistance.toFixed(2)} 的有效突破`}:trend==="偏空"?{do:"宜先看止跌与量价修复，控制试错",avoid:"忌把弱势反弹直接当作趋势反转",focus:`关注 ¥${analysis.support.toFixed(2)} 的承接有效性`}:{do:"宜把区间两端作为观察点，等待方向选择",avoid:"忌在区间中部提前押注方向",focus:`关注 ¥${analysis.support.toFixed(2)} / ¥${analysis.resistance.toFixed(2)} 的突破`};
  const turningWindows=useMemo(()=>{
    const split=Math.max(1,Math.ceil(projectedBars.length*.55)),pick=(bars:typeof projectedBars,offset:number,compare:(next:number,current:number)=>boolean)=>bars.reduce((best,bar,index)=>compare(bar.close,best.bar.close)?{bar,index:index+offset}:best,{bar:bars[0],index:offset});
    const early=projectedBars.slice(0,split),late=projectedBars.slice(Math.max(0,split-1));
    if(!early.length||!late.length)return [];
    const earlyPivot=trend==="偏多"?pick(early,0,(next,current)=>next<current):trend==="偏空"?pick(early,0,(next,current)=>next>current):pick(early,0,(next,current)=>Math.abs(next-(analysis.values.at(-1)||next))<Math.abs(current-(analysis.values.at(-1)||current)));
    const latePivot=trend==="偏多"?pick(late,Math.max(0,split-1),(next,current)=>next>current):trend==="偏空"?pick(late,Math.max(0,split-1),(next,current)=>next<current):pick(late,Math.max(0,split-1),(next,current)=>Math.abs(next-(analysis.values.at(-1)||next))>Math.abs(current-(analysis.values.at(-1)||current)));
    return [{...earlyPivot,label:trend==="偏多"?"回踩承接观察":trend==="偏空"?"反弹压力观察":"区间边界观察"},{...latePivot,label:trend==="偏多"?"突破确认观察":trend==="偏空"?"止跌确认观察":"方向选择观察"}];
  },[analysis.values,projectedBars,trend]);
  const paths=[
  ...
  const ziPing=ziPingFor(listingDate,analysis.score,analysis.momentum,analysis.rsi);
  ...
  const generateShareCard=async()=>{
    ...
    const facts=[["上行确认",`¥${analysis.resistance.toFixed(2)}`],["下行警戒",`¥${analysis.support.toFixed(2)}`],["缠论状态",analysis.chanState],["子平取象",ziPing.label]];
  ...
  const systemShare=async(target:"system"|"douyin"="system")=>{if(!shareImage)return;try{const blob=await(await fetch(shareImage)).blob();const file=new File([blob],`${name||code}-股票推演签.png`,{type:"image/png"});if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({title:`${name||code} 股票推演签`,text:shareCopy,files:[file]});setShareMessage(target==="douyin"?"已打开手机分享面板，请选择抖音":"已打开手机分享面板")}else{downloadShare();await copyShare();setShareMessage(target==="douyin"?"抖音无法被网页直接唤起：分享图已保存、文案已复制，请在抖音上传":"图片已保存，文案已复制")}}catch(error){if((error as Error).name!=="AbortError")setShareMessage(target==="douyin"?"未能唤起抖音：请保存图片后在抖音发布":"分享未完成，请先保存图片")}};
  return <main className="oracle">
    <button className="oracle-theme-toggle" type="button" onClick={toggleUiTheme} aria-label={uiTheme==="dark"?"切换到白天模式":"切换到黑金模式"} title={uiTheme==="dark"?"白天模式":"黑金模式"}>{uiTheme==="dark"?"☀":"☾"}</button>
     <nav><Link href="/">← 返回做T神器</Link><span>股票占卜 · STOCK ORACLE</span><b>娱乐推演</b></nav>
    <header><div><small>东方术数 × 西方星象 × 行情技术</small><h1>股票<br/><span>推演局</span></h1><p>输入代码，查看未来走势、卦象与关键价位。</p></div>
      <form onSubmit={e=>{e.preventDefault();void runReading(false)}}><label>股票代码<input value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,"").slice(0,6))} placeholder="例如 601899"/></label><label>股票名称<input value={name} readOnly placeholder="行情自动识别"/></label><label>问卦周期<select value={horizon} onChange={e=>setHorizon(e.target.value)}><option value="5">短线 · 未来 5 个交易日</option><option value="20">波段 · 未来 20 个交易日</option><option value="60">中期 · 未来 3 个月（约 60 个交易日）</option></select></label><div className="oracle-horizon-tabs" role="group" aria-label="快捷切换推演周期">{horizonOptions.map(option=><button type="button" className={horizon===option.value?"active":""} aria-pressed={horizon===option.value} onClick={()=>setHorizon(option.value)} key={option.value}><b>{option.label}</b><span>{option.detail}</span></button>)}</div><label>上市日期 · {listingDateAuto?"自动识别":"可手动补充"}<input type="date" value={listingDate} onChange={e=>{setListingDate(e.target.value);setListingDateAuto(false)}}/></label><div className={`market-fetch ${marketStatus}`}><i/>{marketMessage}{marketStatus==="ready"&&listingDateAuto?` · 上市 ${listingDate}`:""}</div><button disabled={marketStatus==="loading"||readingStatus==="reading"}>{marketStatus==="loading"?"正在观盘…":readingStatus==="reading"?"正在推演…":`为 ${name||code} 起卦 →`}</button></form>
    </header>
    {readingStatus!=="idle"&&<section className={`reading-ritual ${readingStatus}`} aria-live="polite" aria-busy={readingStatus==="reading"}>
      ...
      <div className="ritual-steps">{[["行情入盘",14],["六爻成卦",34],["子平调候",55],["星盘合参",76],["趋势定签",92]].map(([label,point])=><span className={readingProgress>=Number(point)?"done":""} key={label}><i/>{label}</span>)}</div>
      <div className="ritual-ledger">
        ...
        <span><small>上市命盘取象</small><b>{ziPing.label} · {ziPing.detail}</b></span>
    ...
    <section className={`result ${readingStatus==="reading"?"result-thinking":""}`}>
      ...
      <article className="tech forecast-card"><header><div><small>最终走势</small><h2>{trend}</h2></div><strong>{analysis.score}<i>/100</i></strong></header><div className="meter"><i style={{width:`${analysis.score}%`}}/></div><h3>{futureTitle}</h3><p className="future-path">{futurePath}</p><section className="oracle-action-guide" aria-label="白话操作指引"><small>白话指引 · 仅供文化娱乐</small><div><p><b>宜</b>{actionGuide.do}</p><p><b>忌</b>{actionGuide.avoid}</p><p><b>看</b>{actionGuide.focus}</p></div></section><dl><div><dt>预测周期</dt><dd>未来 {horizon} 个交易日</dd></div><div><dt>风险等级</dt><dd>{riskLevel}</dd></div><div><dt>上行确认</dt><dd>¥{analysis.resistance.toFixed(2)}</dd></div><div><dt>下行警戒</dt><dd>¥{analysis.support.toFixed(2)}</dd></div><div><dt>缠论状态</dt><dd>{analysis.chanState}</dd></div><div><dt>波浪阶段</dt><dd>{analysis.waveState}</dd></div><div><dt>子平取象</dt><dd>{ziPing.label}</dd></div></dl><footer><button type="button" className="share-trigger" onClick={()=>void generateShareCard()}>{shareBusy?"正在生成…":"生成分享签"}</button><div className="result-seal">行情模型定势 · 卦象辅助研判</div></footer></article>
    </section>
    <section className="forecast-chart"><header><div><small>缠论 × 波浪理论 · 推演日线</small><h2>{name||code} · 未来{horizon==="60"?"三个月":`${horizon}日`}路径</h2></div><div className="chart-legend"><span><i/>历史日线</span><span className="chan-line"><i/>缠论笔</span><span className="chan-center"><i/>中枢</span><span className="projected"><i/>模型推演</span><span className="projected-range"><i/>模型区间</span></div></header><canvas ref={chartRef} aria-label={`${name||code}历史日线与未来${horizon}日推演K线图，含缠论笔、中枢和模型区间`}/><footer><span>{analysis.chanState}</span><span>{analysis.waveState}</span><span>推演末值 ¥{(projectedBars.at(-1)?.close||0).toFixed(2)}</span><span>模型区间 ¥{(projectionBand.at(-1)?.low||0).toFixed(2)}–¥{(projectionBand.at(-1)?.high||0).toFixed(2)}</span><span>模型倾向 {analysis.score}/100</span></footer><section className="oracle-turning-windows" aria-label="模型观察时间窗"><header><small>模型观察时间窗</small><span>按交易日计，不对应确定日期</span></header><div>{turningWindows.map((window,index)=><article key={`${window.index}-${window.label}-${index}`}><b>第 {window.index+1} 个交易日前后</b><span>{window.label}</span><em>¥{window.bar.close.toFixed(2)}</em></article>)}</div></section></section>
     {shareImage&&<div className="share-modal" role="dialog" aria-modal="true" aria-label="分享股票推演签" onClick={()=>setShareImage("")}><section onClick={e=>e.stopPropagation()}><header><div><small>分享签已生成</small><h2>发布你的推演结果</h2></div><button type="button" aria-label="关闭" onClick={()=>setShareImage("")}>×</button></header><Image src={shareImage} alt={`${name||code} 股票推演结果分享签`} width={1200} height={1400} unoptimized/><div className="share-actions"><button type="button" className="primary" onClick={()=>void systemShare()}>一键分享</button><button type="button" className="douyin-share" onClick={()=>void systemShare("douyin")}>抖音分享</button><button type="button" onClick={downloadShare}>保存图片</button><button type="button" onClick={()=>void copyShare()}>复制文案</button></div>{shareMessage&&<p aria-live="polite">{shareMessage}</p>}<small>抖音不开放网页定向发布；手机端会打开系统分享面板供你选择抖音，不支持时自动保存图片并复制文案。</small></section></div>}
    <section className="five-elements"><header><div><small>五行审势</small><h2>金木水火土 · 旺衰分布</h2></div><p>由六爻元素分布生成，仅作文化展示。</p></header><div>{(["金","木","水","火","土"] as const).map(element=>{const count=analysis.lines.filter(line=>line.element===element).length;return <article className="element" key={element}><strong>{element}</strong><i><u style={{height:`${24+count*22}%`}}/></i><span><b>{count>=2?"旺":count===1?"平":"弱"}</b><small>{element==="金"?"纪律 / 收敛":element==="木"?"生长 / 趋势":element==="水"?"流动 / 资金":element==="火"?"热度 / 动能":"承载 / 支撑"}</small></span><em>{count}/6</em></article>})}</div></section>
     <section className="rabbit-oracle"><Image src="/rabbit-brand-gold.png" alt="小兔解签官" width={72} height={72}/><div><small>小兔解签官</small><h2>{futureTitle}</h2><p>{spokenReport}</p></div><button type="button" onClick={speak}>🔊 语音播报</button></section>
