...
export default function Home({initialAuth,onLogout,theme:uiTheme,onToggleTheme:toggleUiTheme}:{initialAuth?:InitialAuth;onLogout?:()=>void;theme:UiTheme;onToggleTheme:()=>void}) {
      ...
      :positiveTBlockedByFlow
        ...
        :formalActionMarkerPending
          ...
          :decisionModel.status==="ready"&&decisionActionSide
            ...
            :"等待";
  const formalStrategyCompareTone=decisionModel.status==="locked"||positiveTBlockedByFlow
    ?"negative"
    :decisionModel.status==="ready"&&decisionActionSide
      ?"positive"
      :"waiting";
  ...
  const zijinV29OpeningShadow=useMemo(()=>{
    ...
    if(candidateDirection==="正T"&&positiveTBlockedByFlow)return {
      signal:"否决" as const,
      tone:"warning",
      label:"资金方向否决",
      advice:"暂缓买入",
    ...
    if(l2Confirmed&&l2CandidateDirection)return {
      signal:l2CandidateDirection,
      tone:"confirmed",
      label:`${l2CandidateDirection}信号`,
      advice:l2State==="trigger"?"盘口已触发":"盘口已确认",
      detail:formalAligned
        ?`连续秒级盘口与专属闭环${decisionActionDirection}方向一致`
        :`连续秒级盘口完成确认，由 V2.9 独立输出${l2CandidateDirection}`,
    };
      ...
      <section className={`workspace ${isZijinStock?'with-main-force':''} ${workspaceFullscreen?'workspace-fullscreen':''} ${decisionPanelCollapsed?'decision-panel-collapsed':''} ${decisionZoneMode==="focus"?"decision-focus":"decision-all"} ${chartAnnotationMode==="compact"?"compact-chart-labels":""} ${signalLayerVisible?'':'hide-signal-layer'} ${formalSignalVisible?'':'hide-formal-signal-layer'} ${v29SignalVisible?'':'hide-v29-signal-layer'} ${v1SignalVisible?'':'hide-v1-signal-layer'} ${pricePlanLayerVisible?'':'hide-price-plan-layer'} ${volumeLayerVisible?'':'hide-volume-layer'} ${rabbitTrackerVisible?'':'hide-rabbit-tracker'}`} ref={workspaceRef} style={{"--decision-panel-width":`${decisionPanelWidth}px`,"--order-flow-height":`${orderFlowHeight}px`} as CSSProperties}>
        <div className="chart-zone">
          ...
          <div className="chart-wrap" onWheelCapture={handleIntradayWheel}>
            ...
            {lightweightChartVisible&&<LightweightIntradayChart data={minutePoints.map((point,index)=>{const date=activeChartDate||new Date().toISOString().slice(0,10);const time=String(point.time).padStart(4,"0");const t=Date.UTC(Number(date.slice(0,4)),Number(date.slice(4,6))-1,Number(date.slice(6,8)),Number(time.slice(0,2)),Number(time.slice(2)));const close=Number(point.price);const open=Number(point.open)>0?Number(point.open):Number(minutePoints[index-1]?.price)||close;return {time:Math.floor(t/1000) as any,open,high:Number(point.high)>=Math.max(open,close)?Number(point.high):Math.max(open,close),low:Number(point.low)<=Math.min(open,close)?Number(point.low):Math.min(open,close),close,volume:Number(point.volume)||0,vwap:point.averagePrice??null};})}/>}<svg ref={intradayChartRef} className={`interactive-intraday-chart ${chartPanning?"is-panning":""}`} viewBox={`0 0 ${LIVE_CHART.width} ${LIVE_CHART.height}`} preserveAspectRatio="none" role="img" aria-label={`${activeQuote?.name || stock.name}当日分时图；滚轮或双指缩放，拖动时间轴，双击复位，点击查看分钟详情`} tabIndex={0}
