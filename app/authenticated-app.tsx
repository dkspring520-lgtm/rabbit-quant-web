...
const LIVE_SECOND_CLOCK_FORMATTER = new Intl.DateTimeFormat("en-GB", {
...
const formatLiveSecondTimestamp = (timestamp:number) => {
  const parts=LIVE_SECOND_CLOCK_FORMATTER.formatToParts(new Date(timestamp));
...
type IntradayMinute = {time:string;price:number;volume:number;open?:number|null;high?:number|null;low?:number|null;averagePrice?:number|null;timeVolumeBaseline?:number|null};
...
const calibratedCandidateProbability=(observation:ReplayObservation)=>{
  ...
  if(observation.stage!=="candidate")return null;
  const similarity=observation.similarity;
  if(!similarity?.ready||similarity.samples<30||!Number.isFinite(similarity.hitRate))return null;
  const calibrated=Math.max(0,Math.min(90,Math.round(Number(similarity.hitRate)*20)*5));
  return calibrated>=65?calibrated:null;
};
...
type ReplayMinute = { time:string; price:number; volume:number; high?:number|null; low?:number|null; averagePrice?:number|null };
...
type ZijinV29ReviewSnapshot={
  ...
  testedDays:number;
  l2AvailableDays:number;
  l2MissingDays:number;
  recentDays:number;
  recentCompleted:number;
  recentWinRate:number|null;
  recentProfitFactor:number|null;
  recentNet:number;
  healthStatus:"等待样本"|MultiDayBacktestResult["healthStatus"];
  healthReason:string;
  riskBudgetStatus:MultiDayBacktestResult["riskBudgetStatus"];
  riskBudgetReason:string;
  approvalStatus:MultiDayBacktestResult["approvalStatus"];
  approvalBlockers:string[];
};

const ZIJIN_V29_REVIEW_STORAGE_KEY="rabbit-zijin-v29-review";
const ZIJIN_V29_REVIEW_EVENT="rabbit-zijin-v29-review-updated";
const ZIJIN_MONITOR_STRATEGY_STORAGE_KEY="rabbit-zijin-monitor-strategy";

function readZijinV29ReviewSnapshot(){
  if(typeof window==="undefined")return null;
  try{
    const parsed=JSON.parse(localStorage.getItem(ZIJIN_V29_REVIEW_STORAGE_KEY)??"null") as ZijinV29ReviewSnapshot|null;
    return parsed?.schemaVersion===1?parsed:null;
  }catch{return null;}
}

function persistZijinV29ReviewSnapshot(snapshot:ZijinV29ReviewSnapshot){
  if(typeof window==="undefined")return;
  try{
    localStorage.setItem(ZIJIN_V29_REVIEW_STORAGE_KEY,JSON.stringify(snapshot));
    window.dispatchEvent(new Event(ZIJIN_V29_REVIEW_EVENT));
  }catch{}
}
...
// This is the formal chart projection. L2 repair/order-flow observations use
// the separate shadow layer below and must never be merged into this stream.
function buildReplayChartObservations(code:string|undefined, minutes:ReplayMinute[], observations:ReplayObservation[]) {
  return code === "601899"
    ? mergeZijinReplayObservations(observations) as ReplayObservation[]
...
type IntradayDivergenceResult={
  available?:boolean;
  strength?:number;
  pivot?:{previousIndex?:number;previousTime?:string;previousPrice?:number;confirmationIndex?:number;latestIndex?:number;latestTime?:string;latestPrice?:number;time?:string;price?:number;label?:string;assessment?:"strong"|"confirmed"|"unconfirmed"};
  volumePrice?:{confirmed?:boolean;priceExtensionPct?:number;volumeRatio?:number};
};

type PivotScope = "absolute"|"local";
type PivotRole = "absolute-top"|"absolute-bottom"|"local-resistance"|"local-support";
type PivotPresentation = {
  scope:PivotScope;
  role:PivotRole;
  openingPrice:number|null;
  localPivotPrice:number|null;
  absolutePivotPrice:number|null;
  openingAnchor:boolean;
  pivotConfirmed:boolean;
  probabilityEligible:boolean;
...
function resolveOpeningPrice(minutes:ReplayMinute[],override?:number|null){
  const explicit=finitePositive(override);
  if(explicit!==null)return explicit;
  return finitePositive(minutes.find(point=>isAShareRegularTradingMinute(point.time)&&finitePositive(point.price)!==null)?.price);
}

function causalPriceExtremes(minutes:ReplayMinute[],index:number){
  const causal=minutes.slice(0,Math.max(0,index)+1);
  let high=0;
  let low=Number.POSITIVE_INFINITY;
  for(const point of causal){
    const price=finitePositive(point.price);
    if(price===null)continue;
    const pointHigh=finitePositive(point.high)??price;
    const pointLow=finitePositive(point.low)??price;
    high=Math.max(high,pointHigh);
    low=Math.min(low,pointLow);
  }
  return {high:high>0?high:null,low:Number.isFinite(low)?low:null};
}

function openingExtremeWarning(minutes:ReplayMinute[],index:number,openingPrice:number|null,kind:"top"|"bottom"){
  if(openingPrice===null||index<1)return false;
  const currentPrice=finitePositive(minutes[index]?.price);
  if(currentPrice===null)return false;
  // Only inspect the first twenty points that have already arrived.  When the
  // candidate is still in the opening window, slicing the complete day here
  // would leak later prices into the opening-risk classification.
  const early=minutes.slice(0,Math.min(index+1,20)).map(point=>finitePositive(point.price)).filter((price):price is number=>price!==null);
  const movedAway=kind==="top"
    ?early.some(price=>price<openingPrice*.999)
    :early.some(price=>price>openingPrice*1.001);
  if(!movedAway)return false;
  const extremes=causalPriceExtremes(minutes,index);
  return kind==="top"
    ?currentPrice<openingPrice*.999&&extremes.high!==null&&extremes.high<=openingPrice*1.0005
    :currentPrice>openingPrice*1.001&&extremes.low!==null&&extremes.low>=openingPrice*.9995;
}

function pivotVwapGate(minutes:ReplayMinute[],confirmationIndex:number,pivotIndex:number,price:number,kind:"top"|"bottom"){
  if(!Number.isFinite(price)||price<=0)return false;
  const vwapSeries=cumulativeIntradayAverage(minutes.slice(0,Math.max(confirmationIndex,pivotIndex)+1));
  const vwap=finitePositive(vwapSeries[pivotIndex]??vwapSeries[confirmationIndex]);
  if(vwap===null)return false;
  const deviation=(price-vwap)/vwap;
  return kind==="top"?deviation>=.0015:deviation<=-.0015;
}

function pivotPresentation(observation:ReplayObservation,minutes:ReplayMinute[],openingPriceOverride?:number|null):PivotPresentation{
  const kind=observation.observationKind==="pivot-top"?"top":"bottom";
  // A live/replay quote supplied by the current session is authoritative.  A
  // cached observation may carry an openingPrice from an older render, so it
  // must never override the current opening anchor.
  const openingPrice=finitePositive(openingPriceOverride)??resolveOpeningPrice(minutes)??finitePositive(observation.openingPrice);
  const localPivotPrice=finitePositive(observation.localPivotPrice)??finitePositive(observation.pivotPrice)??finitePositive(observation.price);
  const relation=openingPrice!==null&&localPivotPrice!==null
    ?kind==="top"?localPivotPrice>=openingPrice:localPivotPrice<=openingPrice
    :false;
  const observationIndex=minutes.findIndex(point=>point.time===observation.time);
  const openingPattern=observationIndex>=0&&openingExtremeWarning(minutes,observationIndex,openingPrice,kind);
  const inferredOpeningAnchor=openingPattern&&openingPrice!==null&&localPivotPrice!==null
    &&(kind==="top"?localPivotPrice<=openingPrice*1.0005:localPivotPrice>=openingPrice*.9995);
  // Re-verify the opening-anchor pattern from the causal points.  A persisted
  // flag alone must not revive an old "开盘即高风险" label after the session
  // data or opening quote has been corrected.
  const openingAnchor=openingPrice!==null&&inferredOpeningAnchor;
  // An absolute label is never trusted from persisted data by itself. The
  // current session opening relation is the hard gate; otherwise this is a
  // local support/resistance observation.
  const scope:PivotScope=openingAnchor||relation?"absolute":"local";
  const role:PivotRole=scope==="absolute"
    ?kind==="top"?"absolute-top":"absolute-bottom"
    :kind==="top"?"local-resistance":"local-support";
  // Recompute the absolute anchor from the validated relation instead of
  // trusting a persisted absolutePivotPrice.  This prevents an old/stale
  // value below the open from being rendered as an absolute top (or the
  // symmetric bottom case).
  const absolutePivotPrice=scope==="absolute"
    ?openingAnchor?openingPrice:relation?localPivotPrice:null
    :null;
  const confirmationIndex=minutes.findIndex(point=>point.time===observation.time);
  const pivotIndex=observation.pivotTime
    ?minutes.findIndex(point=>point.time===observation.pivotTime)
    :confirmationIndex;
  const currentVolumeConfirmed=Boolean(observation.volumeConfirmed)
    &&pivotIndex>=0&&confirmationIndex>=0
    &&finitePositive(minutes[pivotIndex]?.volume)!==null
    &&finitePositive(minutes[confirmationIndex]?.volume)!==null;
  const pivotConfirmed=observation.pivotAssessment==="confirmed"||observation.pivotAssessment==="strong";
  const currentVwapConfirmed=scope==="absolute"&&absolutePivotPrice!==null&&pivotIndex>=0&&confirmationIndex>=0
    ?pivotVwapGate(minutes,confirmationIndex,pivotIndex,absolutePivotPrice,kind)
    :false;
  return {
    scope,role,openingPrice,localPivotPrice,absolutePivotPrice,openingAnchor,pivotConfirmed,
    probabilityEligible:Boolean(observation.probabilityEligible)&&scope==="absolute"&&!openingAnchor&&pivotConfirmed&&currentVolumeConfirmed&&currentVwapConfirmed,
    volumeConfirmed:currentVolumeConfirmed,
    vwapConfirmed:currentVwapConfirmed,
  };
}
...
function buildPivotAndMacdObservations(
  ...
  const addPivot=(index:number,result:IntradayDivergenceResult,direction:"正T"|"反T",kind:"pivot-top"|"pivot-bottom")=>{
    ...
    const localPivotPrice=finitePositive(result.pivot.latestPrice)??finitePositive(result.pivot.price)??price;
    const rawPivotIndex=Number.isInteger(Number(result.pivot.latestIndex))
      ?Number(result.pivot.latestIndex)
      :Math.max(0,index-3);
    const pivotIndex=Math.min(index,Math.max(0,rawPivotIndex));
    const relation=openingPrice!==null
      ?kind==="pivot-top"?localPivotPrice>=openingPrice:localPivotPrice<=openingPrice
      :false;
    const openingPattern=openingExtremeWarning(minutes,index,openingPrice,side);
    const openingAnchor=openingPattern
      &&openingPrice!==null
      &&(kind==="pivot-top"?localPivotPrice<=openingPrice*1.0005:localPivotPrice>=openingPrice*.9995)
      &&(kind==="pivot-top"?!openingTopAnchorUsed:!openingBottomAnchorUsed);
    const scope:PivotScope=relation||openingAnchor?"absolute":"local";
    const attemptsForScope=scope==="absolute"?pivotAttempts:localPivotAttempts;
    if(attemptsForScope[side]>=3)return;
    if(index-lastPivotIndex[side][scope]<15)return;
    if(openingAnchor){
      if(kind==="pivot-top")openingTopAnchorUsed=true;
      else openingBottomAnchorUsed=true;
    }
    const absolutePivotPrice=scope==="absolute"?(openingAnchor?openingPrice:localPivotPrice):null;
    const volumeConfirmed=Boolean(result.volumePrice?.confirmed)
      &&finitePositive(minutes[pivotIndex]?.volume)!==null
      &&finitePositive(minutes[index]?.volume)!==null;
    const vwapConfirmed=absolutePivotPrice!==null
    ...
    const pivotTime=String(result.pivot.latestTime??result.pivot.time??minutes[index]?.time??"");
    const label=openingAnchor
      ?kind==="pivot-top"?"开盘即高风险":"开盘即低点"
      :scope==="local"
        ?kind==="pivot-top"?"局部反弹阻力":"局部回踩支撑"
        :probability
          ?`${kind==="pivot-top"?"顶":"底"} ${probability}%`
          :kind==="pivot-top"?"绝对顶观察":"绝对底观察";
    const blockers=["观察层，不触发正式买卖","不计入正式胜率与收益"];
    if(scope==="local")blockers.push(kind==="pivot-top"?"局部阻力，不代表全天绝对顶":"局部支撑，不代表全天绝对底");
    ...
    const observation:ReplayObservation={
      ...
      pivotPrice:localPivotPrice,
      localPivotPrice,
      absolutePivotPrice:absolutePivotPrice??undefined,
...
function observationConfirmationLabel(observation: ReplayObservation,minutes?:ReplayMinute[],openingPriceOverride?:number|null) {
  const presentation=minutes&&observation.observationKind&&observation.observationKind!=="macd"
    ?pivotPresentation(observation,minutes,openingPriceOverride)
    :null;
...
function observationReferencePrice(observation:ReplayObservation,fallback?:number,minutes?:ReplayMinute[],openingPriceOverride?:number|null) {
  const confirmationPrice=Number(observation.price);
  const presentation=minutes&&observation.observationKind&&observation.observationKind!=="macd"
    ?pivotPresentation(observation,minutes,openingPriceOverride)
    :null;
  const pivotPrice=Number(presentation?.localPivotPrice??observation.localPivotPrice??observation.pivotPrice);
  const absolutePivotPrice=Number(presentation?.absolutePivotPrice??observation.absolutePivotPrice);
  const pivotScope=presentation?.scope??observation.pivotScope;
  if((observation.observationKind==="pivot-top"||observation.observationKind==="pivot-bottom")
    &&Number.isFinite(pivotPrice)&&pivotPrice>0){
    if(pivotScope==="absolute"&&Number.isFinite(absolutePivotPrice)&&absolutePivotPrice>0)return absolutePivotPrice;
    // Local structures keep their own pivot price.  Do not replace a local
    // resistance/support with the later confirmation price, or it can cross
    // the opening anchor and look like an absolute top/bottom on the chart.
    return pivotPrice;
  }
...
function topPivotDisplayProbability(value:unknown,eligible=true):TopPivotDisplayProbability|null {
...
export default function Home({initialAuth,onLogout,theme:uiTheme,onToggleTheme:toggleUiTheme}:{initialAuth?:InitialAuth;onLogout?:()=>void;theme:UiTheme;onToggleTheme:()=>void}) {
  ...
  const cockpitLayoutSnapshot=useMemo<CockpitLayoutSnapshot>(()=>({
    ...
    orderFlowHeight,
    viewport:chartViewport,
  }),[chartAnnotationMode,chartViewport,decisionPanelCollapsed,decisionPanelWidth,decisionZoneMode,formalSignalVisible,indicatorsVisible,intradayChartType,orderFlowHeight,pricePlanLayerVisible,rabbitTrackerVisible,signalLayerVisible,v1SignalVisible,v29SignalVisible,volumeLayerVisible]);
  useEffect(()=>{
  ...
  const toggleWorkspaceFullscreen=async()=>{
    ...
    if(!target)return;
    if(workspaceFullscreen&&!document.fullscreenElement){setWorkspaceFullscreen(false);return;}
    ...
    }catch{setWorkspaceFullscreen(true);}
  };
    ...
    const applyPayload=(payload:ZijinL2State)=>{
      ...
      if(payload.status?.connected&&!stale){
        setLiveL2ByMinute(current=>{
          ...
          if(minute&&!(payload.recentMinutes??[]).some(bar=>bar.time===minute))next[minute]=payload;
          return next;
        });
      }
    };
  ...
  const liveQuoteFresh=useMemo(()=>{
    if(liveL2PriceUsable)return true;
    const sourceTimestamp=currentTrial?.sourceTimestamp??currentMarket?.sourceTimestamp??currentTrial?.fetchedAt??currentMarket?.fetchedAt;
  ...
  },[activeQuote?.price,liveQuoteFresh]);
  useEffect(()=>{
    const shouldCollect=activeView==="操盘台"&&stock?.code==="601899"&&marketSession.live;
    if(!shouldCollect){
      liveSecondQuoteRef.current={price:null,fresh:false};
      setLiveSecondPoints(current=>current.length?[]:current);
      return;
    }
    let active=true;
    const capture=()=>{
      if(!active)return;
      const {price,fresh}=liveSecondQuoteRef.current;
      if(!fresh||price===null)return;
      const timestamp=Date.now();
      const {date,time}=formatLiveSecondTimestamp(timestamp);
      if(!isAShareRegularTradingMinute(time.slice(0,4)))return;
      setLiveSecondPoints(current=>{
        const last=current.at(-1);
        if(last?.date!==date)current=[];
        const previous=current.at(-1);
        const point={date,time,timestamp,price};
        if(previous?.time===time){
          if(Math.abs(previous.price-price)<1e-9)return current;
          return [...current.slice(0,-1),point];
        }
        const next=[...current,point];
        return next.length>15_000?next.slice(-15_000):next;
      });
    };
    capture();
    const timer=window.setInterval(capture,1_000);
    return()=>{active=false;window.clearInterval(timer)};
  },[activeView,marketSession.live,stock?.code]);
  useEffect(()=>{
  ...
  const minutePoints = useMemo(() => {
    type LiveMinutePoint = {
      time:string; price:number; volume:number; open?:number|null; high?:number|null; low?:number|null; averagePrice?:number|null;
  ...
  const chartModel = useMemo(() => {
    ...
    const candleReady=candles.length>=Math.max(2,Math.ceil(minutePoints.length*.8));
  ...
  const liveSecondChart=useMemo(()=>{
    ...
    for(const point of liveSecondPoints){
      ...
      current.push({x,y,price:point.price,time:point.time});
      previous=point;
    }
    if(current.length>0){
      const last=current.at(-1)!;
  ...
  const zijinVisibleFootprint=useMemo(()=>{
    ...
    return [...rows]
      .sort((left,right)=>Math.abs(Number(left.price)-reference)-Math.abs(Number(right.price)-reference))
      .slice(0,7)
  ...
  useEffect(()=>{
    const timer=window.setTimeout(()=>{
      ...
      if(activeView==="操盘台")setChartViewport({start:0,span:COCKPIT_VIEWPORT_FULL_SPAN});
    },0);
    return()=>window.clearTimeout(timer);
  },[activeView,stock?.code]);
  ...
  const zijinV29Replay=useMemo<BacktestResult|null>(()=>{
    ...
    const matchingPreopenPlan=frozenZijinPreopenPlan?.date===replayDate
      ...
      :null;
    try{
      return runZijinV29ShadowReplay({
        code:"601899",
        date:replayDate,
        previousClose:activeQuote?.previousClose??null,
        minutes:minutePoints,
        ...
        preopenPlan:matchingPreopenPlan,
      }) as BacktestResult;
    }catch{
      return null;
    }
  ...
  const zijinV29ChartObservations=useMemo(()=>{
    ...
    return (zijinV29Replay.observations??[])
      ...
      .map(observation=>({
        ...observation,
        executable:false,
        confirmationLabel:observation.direction==="反T"?"V2.9 反T观察":"V2.9 正T观察",
  ...
  const reverseTSignalLabel=freshReverseTAction
    ...
    : reverseTDisplayObservation
      ? observationConfirmationLabel(reverseTDisplayObservation,minutePoints,activeQuote?.open)
      : "暂无候补";
  ...
  const visibleChartObservations=useMemo<ChartObservation[]>(()=>{
    ...
    return [
      ...compactTagged(currentObservations,"closure",isZijinStock),
  ...
  },[causalObservationLayer,currentObservations,isZijinStock,zijinV1ChartObservations,zijinV29ChartObservations]);
  ...
  const durableVisibleChartObservations=useMemo<ChartObservation[]>(()=>{
    ...
    const activeMacdKeys=new Set(causalObservationLayer
      .filter(observation=>observation.observationKind==="macd")
      .map(observationKey));
    const persistedEligible=persistedChartObservations.observations
  ...
  },[causalObservationLayer,chartObservationStorageKey,isZijinStock,persistedChartObservations,visibleChartObservations]);
  ...
  const rabbitTrackerSignal=useMemo(()=>{
    ...
    const probability=calibratedCandidateProbability(recentObservation);
    return {
      key:`observation-${recentObservation.time}-${rawLabel}`,
      label:`${compactIntradayPrompt(rawLabel)}${probability===null||rawLabel.includes("%")?"":` ${probability}%`}`,
      tone:"watch",
  ...
  const intradayMarkerLayout=useMemo(()=>{
    if(!chartModel)return {observations:[],tooltipObservations:[],actions:[],shadowActions:[],rabbitCandidates:[],manualTrades:[],riskMarkers:[],fusionMarkers:[]};
    ...
    const reserveDirectionalMarkerLabel=(pointX:number,pointY:number,width:number,height:number,isSell:boolean,required=false)=>{
      ...
      const preferredAbove=isSell;
      // Keep every badge inside the price plot. The old 13..245 clamp caused
      // all labels near the top/bottom edge to collapse onto one baseline.
      const baselineMin=LIVE_CHART.priceTop+4+height-1;
      const baselineMax=LIVE_CHART.priceBottom-4-6;
      const rowStep=Math.max(18,height+7);
      const rows:number[]=[];
      ...
      for(const labelX of labelXs){
        for(const baseline of orderedBaselines){
          const box=makeBox(labelX,baseline);
          const collision=collides(box);
          if(!collision){
            occupied.push({left:box.left-8,right:box.right+8,top:box.top-8,bottom:box.bottom+8});
            return {labelX,labelY:baseline,labelAbove:baseline<pointY,labelRendered:true};
          }
          if(required){
            const overlap=occupied.reduce((total,other)=>total+Math.max(0,Math.min(box.right,other.right)-Math.max(box.left,other.left))*Math.max(0,Math.min(box.bottom,other.bottom)-Math.max(box.top,other.top)),0);
            const candidate={labelX,labelY:baseline,box,overlap,distance:Math.abs(labelX-pointX)+Math.abs(baseline-preferredTarget)};
            if(!best||candidate.overlap<best.overlap||candidate.overlap===best.overlap&&candidate.distance<best.distance)best=candidate;
          }
        }
      }
      if(best){
        occupied.push({left:best.box.left-8,right:best.box.right+8,top:best.box.top-8,bottom:best.box.bottom+8});
        return {labelX:best.labelX,labelY:best.labelY,labelAbove:best.labelY<pointY,labelRendered:true};
      }
      // A lower-priority marker keeps its anchor/title but never forces a
      // second badge onto an occupied lane.
      return {labelX:pointX,labelY:pointY,labelAbove:preferredAbove,labelRendered:false};
    };
    ...
    const manualTrades=visibleManualTradeRows.flatMap((row,index)=>{
      ...
      const placed=reserveDirectionalMarkerLabel(point.x,point.y,20,18,isSell,true);
    ...
    const shadowActions=selectedShadowActions.flatMap(({action,strategy},index)=>{
      ...
      // A shadow anchor without its badge is not actionable and reads as a
      // floating yellow dot. Keep the full action in replay data, but do not
      // paint an orphan point on the live chart.
      if(!placed.labelRendered)return [];
      labeledSignalEpisodes.push({time:action.time,isSell});
    ...
    const observations=orderedDurableObservations.flatMap((observation,index)=>{
      ...
      // These are explanatory/research states, not executable signals.  The
      // persisted replay can still keep them for audit, but the live chart
      ...
      if(hideAuxiliaryObservation||hideNonActionableObservation)return [];
    ...
    const recordedCandidates=(isZijinStock
      ...
      return [{...point,labelX:point.x,labelY:point.y,labelAbove:isSell,labelRendered:false,isSell,label,labelWidth:label.length*8+16,time,price:markerPrice,key:`recorded-${alert.eventKey??alert.id??index}`}];
    });
    ...
    const rabbitCandidates=rabbitTrackerSignal?.source==="displacement"
      ?(()=>{
          ...
          return [{...point,labelX:point.x,labelY:point.y,labelAbove:isSell,labelRendered:false,isSell,label,labelWidth:label.length*8+16,time:rabbitTrackerSignal.time,price:rabbitTrackerSignal.price,key:rabbitTrackerSignal.key}];
        })()
      :[];
  ...
  const intradayCursorSignal=useMemo(()=>{
    ...
    if(observation)return observationConfirmationLabel(observation,minutePoints,activeQuote?.open);
  ...
  },[activeQuote?.open,intradayCursor,intradayMarkerLayout,minutePoints]);
  const signalFunnel = (() => {
  ...
  const openingStageCard=(()=>{
    ...
    if(!opened){
      ...
      if(!postclose&&preopenExternalImpact.forceWeak)return {
        ...
        tone:"down",
        ariaLabel:"盘前外盘冲击警告",
        tooltip:"新鲜纽约金跌幅超过 2.5%，且至少一项跨市场数据确认，盘前结论强制降为偏弱/高风险；这不是直接反T指令，09:30 后由实际开盘、VWAP、成交量与量价结构判断，订单流仅作影子观察。",
      };
      if(!postclose&&preopenExternalImpact.shock)return {
        ...
        tooltip:"只有黄金单项显著下跌，因此只压制偏强预判，不生成买卖指令；09:30 后由实际开盘、VWAP、成交量与量价结构判断，订单流仅作影子观察。",
      };
  ...
  const formalActionMarkerPending=Boolean(decisionModel.status==="ready"&&latestFormalAction&&!latestFormalActionMarked);
    ...
    :cycleStage==="opened"
      ...
      :formalActionMarkerPending
        ...
        :decisionModel.status==="ready"&&decisionActionSide
          ...
          :"等待";
  const formalStrategyCompareTone=decisionModel.status==="locked"
    ?"negative"
    :decisionModel.status==="ready"&&decisionActionSide
      ?"positive"
      :"waiting";
  ...
  const [flowDetail,setFlowDetail]=useState<{kind:'current'|'history';title:string;time?:string;price?:number;score?:number;reasons?:string[];layers?:{type:string;label:string;score?:number;reasons:string[]}[]}|null>(null);
  ...
  const groupedChartEvents=useMemo(()=>groupChartEvents(pathEvents,openingChartScores,sliceChartEvents),[pathEvents,openingChartScores,sliceChartEvents]);
  ...
  const zijinV29OpeningShadow=useMemo(()=>{
    ...
    const l2Confirmed=Boolean(l2Usable&&(l2State==="ready"||l2State==="trigger"));
    const formalAligned=Boolean(decisionActionSide&&l2Confirmed&&decisionActionSide===l2Direction);
    const l2CandidateDirection=l2Direction==="buy"?"正T":l2Direction==="sell"?"反T":null;
    const candidateDirection=l2CandidateDirection??decisionActionDirection??(
      signalMode==="正T"||signalMode==="反T"?signalMode:null
    );
    ...
    if(l2Confirmed&&l2CandidateDirection)return {
      signal:`${l2CandidateDirection}观察`,
      tone:"confirmed",
      label:`${l2CandidateDirection}影子观察`,
      advice:l2State==="trigger"?"盘口影子触发":"盘口影子确认",
      detail:formalAligned
        ?`连续秒级盘口与专属闭环${decisionActionDirection}方向一致；仅记录，不改变正式闭环`
        :`连续秒级盘口形成 V2.9 ${l2CandidateDirection}影子观察；不触发、不拦截正式信号`,
    };
  ...
  },[decisionActionDirection,decisionActionSide,decisionModel.confirmed,decisionModel.mode,isZijinStock,liveL2HasTicks,liveL2Stale,secondLevelSignal,signalFunnel.currentObservations,signalMode,zijinPreopenGate]);
  ...
  const decisionConditions=useMemo(()=>{
    ...
    const structureConfirmed=decisionModel.status==="ready"
      || (decisionModel.referenceConfirmed&&decisionModel.trendConfirmed);
    return [
      ...
      {label:"量价结构",met:structureConfirmed},
    ];
  ...
  const executionSnapshot=useMemo(()=>{
    const plan=displayedZijinPricePlan;
    if(!plan?.ready||!("riskPlan" in plan)||!plan.riskPlan)return null;
  ...
  const deskHistoryRows=useMemo<DeskHistoryRow[]>(()=>{
    ...
    if(panel==="历史信号"){
      const observationRows=[...currentObservations].reverse().map(observation=>{
        ...
        return {time,direction:observationConfirmationLabel(observation,minutePoints,activeQuote?.open),price:point?point.price.toFixed(2):"—",quantity:"未下单",spread:`预估 ${observation.edge.toFixed(2)}%`,status:`${observationDirectionNote(observation)}；${qualified?"候选门槛通过":observation.blockers[0]??"尚未达到候选门槛"}`,tone:"candidate" as const};
      });
  ...
  },[activeQuote?.open,panel,confirmedCycleRows,visibleStockAgentEvaluation,currentObservations,minutePoints,liveEngine.actions]);
  useEffect(() => {
  ...
  useEffect(()=>{
    ...
    for(const [index,item] of stockList.entries()){
      ...
      const engineCandidateProbability=selectedEngineCandidate?calibratedCandidateProbability(selectedEngineCandidate):null;
        ...
        : formalFresh
          ...
          : selectedExperimental
            ...
            : selectedAgent
              ...
              : `${item.name} ${selectedEngineCandidate!.direction==="反T"?"候卖":"候买"}${engineCandidateProbability===null?"":` ${engineCandidateProbability}%`}`;
  ...
  const tShareObservationSignals=useMemo(()=>{
    ...
    return [
      ...intradayMarkerLayout.observations.map(marker=>({
        ...
        price:observationReferencePrice(marker.observation,Number(marker.observation.price),minutePoints,activeQuote?.open),
      ...
      <section className={`workspace ${isZijinStock?'with-main-force':''} ${workspaceFullscreen?'workspace-fullscreen':''} ${decisionPanelCollapsed?'decision-panel-collapsed':''} ${decisionZoneMode==="focus"?"decision-focus":"decision-all"} ${chartAnnotationMode==="compact"?"compact-chart-labels":""} ${signalLayerVisible?'':'hide-signal-layer'} ${formalSignalVisible?'':'hide-formal-signal-layer'} ${v29SignalVisible?'':'hide-v29-signal-layer'} ${v1SignalVisible?'':'hide-v1-signal-layer'} ${pricePlanLayerVisible?'':'hide-price-plan-layer'} ${volumeLayerVisible?'':'hide-volume-layer'} ${rabbitTrackerVisible?'':'hide-rabbit-tracker'}`} ref={workspaceRef} style={{"--decision-panel-width":`${decisionPanelWidth}px`,"--order-flow-height":`${orderFlowHeight}px`} as CSSProperties}>
        <div className="chart-zone">
          ...
          <div className="chart-wrap" onWheelCapture={handleIntradayWheel}>
            ...
            <div className={`intraday-hud ${chartHud.tone}`} aria-live="polite">
              ...
              {(zijinChartPriceOverlay?.hiddenCount??0)>0&&<button onClick={()=>setShowAllPriceLevels(value=>!value)}>{showAllPriceLevels?"只看最近2条":`展开全部 +${zijinChartPriceOverlay?.hiddenCount??0}`}</button>}
            </div>
            {false&&<LightweightIntradayChart data={minutePoints.map((point,index)=>{const date=(activeChartDate||new Date().toISOString()).replace(/\D/g,"").slice(0,8);const time=String(point.time).replace(/\D/g,"").slice(-4).padStart(4,"0");const t=Date.UTC(Number(date.slice(0,4)),Number(date.slice(4,6))-1,Number(date.slice(6,8)),Number(time.slice(0,2)),Number(time.slice(2)));const close=Number(point.price);const open=Number(point.open)>0?Number(point.open):Number(minutePoints[index-1]?.price)||close;return {time:Math.floor(t/1000),open,high:Number(point.high)>=Math.max(open,close)?Number(point.high):Math.max(open,close),low:Number(point.low)<=Math.min(open,close)?Number(point.low):Math.min(open,close),close,volume:Number(point.volume)||0,vwap:point.averagePrice??null};})}/>}<svg ref={intradayChartRef} className={`interactive-intraday-chart ${chartPanning?"is-panning":""}`} viewBox={`0 0 ${LIVE_CHART.width} ${LIVE_CHART.height}`} preserveAspectRatio="none" role="img" aria-label={`${activeQuote?.name || stock.name}当日分时图；滚轮或双指缩放，拖动时间轴，双击复位，点击查看分钟详情`} tabIndex={0}
              ...
              {intradayMarkerLayout.fusionMarkers.map(marker=><g key={`fusion-${marker.time}`} className={`candidate-signal-marker fusion-marker ${marker.side}`}><title>{`${marker.time.slice(0,2)}:${marker.time.slice(2)} · ${marker.label} · ${marker.detail}`}</title><circle cx={marker.x} cy={marker.y} r="6"/><text x={marker.x+8} y={marker.y-8}>{marker.label}</text></g>)}
        ...
        <aside className={`decision-zone ${decisionZoneMode==="focus"?"focus-mode":"all-mode"}`}>
          {isZijinStock&&<section className={`order-flow-top-card ${orderFlowCurrentAvailable?"ready":"waiting"} ${orderFlowFormalLink.state}`} aria-label="双兔订单流影子行为面板">
            ...
            {flowDetail?.kind==='history'?<section aria-label="历史事件证据"><p>历史观察事件 · {activeChartDate??'交易日待核验'} {flowDetail.time}</p><p>价格 {flowDetail.price?.toFixed(2)}{flowDetail.score!=null?` · ${flowDetail.score}分`:null}</p>{flowDetail.layers?.length?<div>{flowDetail.layers.map((layer,index)=><section key={`${layer.type}:${index}`}><h3>{layer.type==='score'?'买卖观察评分':layer.type==='observation'?'开盘 / 路径观察评分':'路径结构'}</h3><p>{layer.label}{layer.score!=null?` · ${layer.score}分`:''}</p>{layer.reasons.length?<ul>{layer.reasons.map(reason=><li key={reason}>{reason}</li>)}</ul>:<p>该层未保存逐项评分依据，不用当前数据补填。</p>}</section>)}</div>:flowDetail.reasons?.length?<ul>{flowDetail.reasons.map(reason=><li key={reason}>{reason}</li>)}</ul>:<p>该历史事件未保存逐项评分依据，不使用当前盘口补填。</p>}<p>失效条件：该事件未保存明确失效阈值，不能据此判断当前仍有效。</p><small>分数来自历史分钟重建，非成交记录；仅观察，不触发语音。</small></section>:<>
            ...
            {orderFlowCurrentAvailable?<>
              ...
              <div className="order-flow-behavior-grid detailed" aria-label="订单流行为传感器">
                ...
                <span title={Number.isFinite(orderFlowEfficiencyValue)?"近5分钟价格变动（bp）/ 每百万元净主动额；只作结构观察。":"等待可比较的价格与主动Delta。"}><em>推进效率</em><b>{Number.isFinite(orderFlowEfficiencyValue)?`${String(zijinOrderFlowRadar.efficiency?.label??"--")} · ${orderFlowEfficiencyValue.toFixed(1)}bp/百万`:String(zijinOrderFlowRadar.efficiency?.label??"待数据")}</b></span>
                <span><em>成交强度</em><b>{web4Microstructure.behavior.intensity.available&&web4Microstructure.behavior.intensity.value!==null?`${web4Microstructure.behavior.intensity.value.toFixed(1)}×`:"待数据"}</b></span>
          ...
          <section className={`web4-monitor ${displayedWeb4Status} ${displayedWeb4Status==="degraded"||auctionDataPending||postcloseDataPending?"compact":""} ${marketSession.live?"market-live":""}`} aria-label="WEB 4.0 多源实时监控">
            ...
            {auctionDataPending||postcloseDataPending
              ...
              :<footer><b>{web4Monitor.shadowAligned?"多源影子观察一致":"仅监控，不升级正式信号"}</b><span>{web4Monitor.summary}</span></footer>}
          </section>
...
function OnboardingView({accountName,initial,initialList,initialPositions,maxStocks,onSave}:{accountName:string;initial:AccountPreferences;initialList:typeof initialStocks;initialPositions:StockPositionMap;maxStocks:number;onSave:(value:AccountPreferences,list:typeof initialStocks,positions:StockPositionMap)=>Promise<void>}){
  ...
  const updatePositionDraft=(field:"tradeShares"|"maxDailyTrades",rawValue:string)=>{
    const digits=rawValue.replace(/\D/g,'');
    setPositionDrafts(current=>({...current,[selectedCode]:{...current[selectedCode],[field]:digits}}));
    updatePosition(field,field==="maxDailyTrades"?(Number(digits)||1):(Number(digits)||0));
  };
  const finishPositionDraft=(field:"tradeShares"|"maxDailyTrades")=>setPositionDrafts(current=>{
    const fields={...current[selectedCode]};
    delete fields[field];
    const next={...current};
    if(Object.keys(fields).length)next[selectedCode]=fields;
...
type ZijinL2State = {
  ...
  recentMinutes?:{time:string;exchangeMinute?:string;price:number;open?:number;high?:number;low?:number;volume?:number;amount?:number;averagePrice?:number|null;activeBuyNotional?:number;activeSellNotional?:number;activeBuyVolume?:number;activeSellVolume?:number;activeBuyRatio?:number|null;netActiveNotional?:number;bigBuyNotional?:number;bigSellNotional?:number;bigOrderNetNotional?:number;bigBuyVolume?:number;bigSellVolume?:number;bigBuyCount?:number;bigSellCount?:number;footprint?:L2FootprintLevel[]}[];
  l2Bar?:{time:string;exchangeMinute?:string;price:number;open?:number;high?:number;low?:number;volume?:number;amount?:number;averagePrice?:number|null;activeBuyNotional?:number;activeSellNotional?:number;activeBuyVolume?:number;activeSellVolume?:number;activeBuyRatio?:number|null;netActiveNotional?:number;bigBuyNotional?:number;bigSellNotional?:number;bigOrderNetNotional?:number;bigBuyVolume?:number;bigSellVolume?:number;bigBuyCount?:number;bigSellCount?:number;footprint?:L2FootprintLevel[]};
...
const AI_RESEARCH_FLOW=[
  {name:"提交研究",detail:"因子或参数"},
  {name:"离线验证",detail:"样本外 · 扣费后"},
  {name:"风险复核",detail:"回撤 · 稳定性"},
        ...
        try{
          ...
          strictL2={
            ...
            reason:l2Payload.available
              ? observations.length?`L2影子层已加载，并复现 ${observations.length} 个独立资金修复阶段`:"L2影子层已加载，本日没有形成持续资金修复观察"
              : l2Payload.reason??"本交易日没有历史L2快照；正式回放不受影响",
          };
        }catch{
          ...
          const loaded=await Promise.all(batch.map(async session=>{
            try{
              ...
              return {
                ...
                l2State:{available:true,source:l2Payload.source??"archive",minuteCount:l2Minutes.length,observations,reason:observations.length?`L2影子层已加载，并复现 ${observations.length} 个独立资金修复阶段`:"L2影子层已加载，本日没有形成持续资金修复观察"} as L2ReplayState,
              };
            }catch{return null;}
          }));
    ...
    <div className="backtest-grid">
      ...
      <div className={`backtest-results ${trainingMode?"training-active":""}`} id="single-backtest-result">
        ...
        {multiDay?.l2Required&&<details className="candidate-audit">
          ...
          <p className="candidate-audit-foot">正式策略使用开盘方向锚、大盘/板块、VWAP、量价结构与成本风控；L2/OFI 仅作影子质量观察。风险预算继续执行费用、滑点、仓位、连续失败和尾盘复位约束。</p>
        </details>}
        ...
        <div className="equity-panel">
          ...
          {result&&source?.quote.code==="601899"&&<div className={`l2-replay-audit ${l2Replay.available?"available":"unavailable"}`}><span><i/>L2影子因果回放</span><b>{l2Replay.reason}</b><em>{l2Replay.available?`${l2Replay.minuteCount} 个L2分钟点 · 识别 ${visibleL2ReplayMarkers.length} 个关键影子波段 · ${l2Replay.source==="archive"?"交易日归档":"当日实时快照"}`:"未使用L2补值；正式回放不受影响"}</em></div>}
          {result&&source?.quote.code==="601899"&&<div className={`replay-order-flow-summary ${replayOrderFlowRadar?.available?"available":"unavailable"}`}><span>订单流影子评分</span><b>{replayOrderFlowRadar?.available?`正T ${replayOrderFlowRadar.scores.lowBuy}分 · 反T ${replayOrderFlowRadar.scores.takeProfit}分`:"回放无当前分钟L2"}</b><small>{replayOrderFlowRadar?.available?`Delta 1/3/5分：${[replayOrderFlowRadar.delta.oneMinute,replayOrderFlowRadar.delta.threeMinute,replayOrderFlowRadar.delta.fiveMinute].map(value=>value==null?"--":formatMainForceAmount(Number(value))).join(" · ")} · ${replayOrderFlowRadar.absorption.label} · ${replayOrderFlowRadar.divergence.label} · 不影响正式闭环`:"未使用公开行情补造评分"}</small></div>}
            ...
            {points&&<>
              ...
              {visibleReplayChartObservations.map((observation,index)=>{
                ...
                const price=observation.observationKind
                  ?observationReferencePrice(observation,fullDayMinutes[minuteIndex].price,fullDayMinutes,source?.quote.open) ?? fullDayMinutes[minuteIndex].price
                  :observation.coverageOnly&&Number.isFinite(observation.pivotPrice)
                  ? observation.pivotPrice
                  : observation.price ?? fullDayMinutes[minuteIndex].price;
                const point=chartPoint(Number.isFinite(Number(price))?Number(price):fullDayMinutes[minuteIndex].price,minuteIndex);
                const isSell=observation.direction==="反T";
                const label=observationConfirmationLabel(observation,fullDayMinutes,source?.quote.open);
        ...
        {result&&<details className="candidate-audit" key={`candidate-audit-${singleRunDate}`} open={result.trades===0 || undefined}>
          ...
          {visibleBacktestObservations.length>0?<div className="candidate-audit-list">{visibleBacktestObservations.map((observation,index)=>{
            ...
            return <article key={`${observation.direction}-${observation.time}-${index}`} className={observation.executable?"passed":observation.stage==="candidate"?"candidate":"watch"}>
              <header><span><i>{observation.l2Strict?"L2严格":observation.coverageOnly&&source?.quote.code==="601899"?"紫金候选":observation.coverageOnly?"覆盖":observation.stage==="candidate"?"候选":"观察"}</i><b>{formatTime(observation.time)} · {observationConfirmationLabel(observation,fullDayMinutes,source?.quote.open)}</b></span><em>{observation.l2Strict?`${observation.score}/${observation.threshold} 分 · 历史L2因果确认`:observation.coverageOnly?"研究候选 · 不计胜率":`${observation.score}/${observation.threshold} 分 · 预估价差 ${observation.edge.toFixed(2)}%`}</em></header>
