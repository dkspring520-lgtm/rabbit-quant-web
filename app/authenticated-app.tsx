...
export default function Home({initialAuth,onLogout,theme:uiTheme,onToggleTheme:toggleUiTheme}:{initialAuth?:InitialAuth;onLogout?:()=>void;theme:UiTheme;onToggleTheme:()=>void}) {
  ...
  const intradayMarkerLayout=useMemo(()=>{
    ...
    const shadowActions=(zijinMonitorStrategy==="v29"?(zijinV29Replay?.actions??[]):[]).flatMap((action,index)=>{
      ...
      const placed=reserveLabel(point.x,isSell?point.y-13:point.y+22,labelWidth,17,isSell?-1:1);
      return [{...point,...placed,index,isSell,label,labelWidth,action}];
    });
        ...
        <div className="chart-zone">
          ...
          <div className="chart-wrap">
            ...
            <svg ref={intradayChartRef} className="interactive-intraday-chart" viewBox={`0 0 ${LIVE_CHART.width} ${LIVE_CHART.height}`} preserveAspectRatio="none" role="img" aria-label={`${activeQuote?.name || stock.name}当日分时图；移动鼠标或拖动手指查看分钟详情`} tabIndex={0}
              ...
              {liveSecondChart&&<g className="live-second-layer" aria-label={`秒级观察轨迹，共 ${liveSecondPoints.length} 个有效报价点`}>{liveSecondChart.segments.map((segment,index)=><polyline key={index} points={segment.points} className="live-second-path"/>)}<circle cx={liveSecondChart.last.x} cy={liveSecondChart.last.y} r="3.2" className="live-second-dot"><title>{`${liveSecondChart.last.time.slice(0,2)}:${liveSecondChart.last.time.slice(2,4)}:${liveSecondChart.last.time.slice(4)} · 秒级观察 ${liveSecondChart.last.price.toFixed(2)}`}</title></circle></g>}
