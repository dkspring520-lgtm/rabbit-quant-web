...
function TrainingView({evidence,accountName,stock,position,premiumEnabled,onOpenAccount}:{evidence:{sessions:number;cycles:number;wins:number;net:number;maxDrawdown:number;confidence:string;winRate:number|null};accountName:string;stock:{code:string;name:string};position:StockPosition;premiumEnabled:boolean;onOpenAccount:()=>void}) {
  ...
  return <section className="module-view training-view">
    ...
    />
    <div className="lab-grid">{agents.map((agent,index)=>{const isTraining=agent.id==="training";const isChallenger=agent.id==="challenger";const isRisk=agent.id==="risk";const value=isTraining?sampleCoverage:isChallenger?validationCoverage:isRisk?Math.min(100,evidence.maxDrawdown*1000):(canReview?100:0);const label=isTraining?`${evidence.sessions}/20 日`:isChallenger?`${evidence.cycles}/20 闭环`:isRisk?`${(evidence.maxDrawdown*100).toFixed(2)}%`:(canReview?'可评审':'正式版锁定');return <article className={`lab-agent ${isRisk&&evidence.maxDrawdown<.03?'risk-safe':''}`} key={agent.name}><div><span className={`agent-icon a${index}`}><Image src={agent.avatar} alt={`${agent.name} AI头像`} width={40} height={40}/></span><p><b>{agent.name}</b><small>{isTraining?'候选参数研究':isChallenger?'未见样本验证':isRisk?'费用与风险审计':'正式版本门控'}</small></p><em>{isTraining?'本股证据已读取':isChallenger?'等待通用盲测':isRisk?(evidence.maxDrawdown<.03?'本股风控绿灯':'本股风控关注'):'需人工批准'}</em></div><strong>{label}<small>{isTraining?'当前股票交易日':isChallenger?'当前股票扣费闭环':isRisk?'当前股票最差回撤':'自动晋升关闭'}</small></strong><i><span style={{width:`${value}%`}}/></i><p>{isTraining?'通用训练应使用历史全市场数据；这里仅显示当前股票证据。':isChallenger?'候选必须换未见股票与日期考试，不能只背训练样本。':isRisk?'费用、滑点、回撤、PBO 和 DSR 任一不合格即可否决。':'只有通过盲测、风控和影子核对后，才允许人工评审。'}</p></article>})}</div>
