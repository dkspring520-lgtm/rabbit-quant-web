...
function BatchReport({batch,representativeCode}:{batch:BatchBacktestResult;representativeCode?:string}) {
  ...
  return <section className="batch-report" aria-label="随机10股真实分时批次汇总">
    ...
    <p>每次点击都先对当前全 A 股普通股票池重新洗牌并无放回抽取 10 只；最近 6 批已经出现的股票会排到队尾，行情缺失再从全市场继续补抽。只有全市场列表暂时不可用时才明确回退代表池。每股图上最多保留 2 个低位/反弹参考和 2 个高位/回落参考，正式候选与正式闭环另行标注。</p>
  </section>;
}
...
function SingleStockResearchView({accountName,stock,quote,marketData,profile,profitMode,position,manualCount,onOpenConsole}:{accountName:string;stock:{code:string;name:string;price:string;change:string};quote:MarketData['quote']|undefined;marketData:MarketData|null;profile:string;profitMode:ProfitMode;position:StockPosition;manualCount:number;onOpenConsole:()=>void}) {
  ...
  return <section className="stock-research-view">
    ...
    {stock.code==="601899"&&<article className={`research-compact-training ${zijinL2?.status?.connected&&!zijinL2.status.stale?"completed":"stale"}`}><div><span>L2 实时监控主源 · 上海节点</span><b>{zijinL2?.status?.authorized===false?"账号暂无 601899 主题权限":zijinL2?.status?.connected?(zijinL2.status.stale?"已连接，等待新盘口":`十档与逐笔正常 · 最新价 ${zijinL2.book?.lastPrice?.toFixed(2)??"--"}`):"正在连接 quote5.base32.cn"}</b><small>主动买占比 {zijinL2?.flow?.activeBuyRatio60s==null?"--":`${(zijinL2.flow.activeBuyRatio60s*100).toFixed(1)}%`} · 五档失衡 {zijinL2?.book?.nearTouchImbalance==null?"--":zijinL2.book.nearTouchImbalance.toFixed(3)} · 腾讯分钟线自动兜底 · 前瞻样本 {zijinL2?.forward?.samples??0}</small></div><strong>{zijinL2?.status?.stale?"等待":"L2"}</strong></article>}
