...
export default function Home() {
  ...
  useEffect(()=>{
    ...
    for(const [index,item] of stockList.entries()){
      ...
      const rabbit=isRisk?"both":formalFresh?(latest!.side.includes("卖")?"sell":"buy"):(latestObservation!.direction==="反T"?"sell":"buy");
...
function SingleStockResearchView({accountName,stock,quote,marketData,profile,position,manualCount,onOpenConsole}:{accountName:string;stock:{code:string;name:string;price:string;change:string};quote:MarketData['quote']|undefined;marketData:MarketData|null;profile:string;position:StockPosition;manualCount:number;onOpenConsole:()=>void}) {
  ...
  return <section className="stock-research-view">
    ...
    {stock.code==="601899"&&<div id="zijin-experiment-progress" className={`zijin-training-live zijin-training-prominent ${zijinTrainingProgress?.status??'loading'}`}>
      <div className="zijin-training-title"><div><span><i/>紫金矿业 · 四兔真实训练进度</span><b>{zijinTrainingProgress?.message??'正在连接服务器训练记录…'}</b></div><strong>{zijinTrainingProgress?`${zijinTrainingProgress.progress}%`:'--'}</strong></div>
    ...
    {zijinFactorResearch&&<section className="zijin-factor-lab">
      ...
      <div className={`zijin-pattern-result ${zijinPatternDiscovery.conclusion.status}`}>
        <div className="zijin-pattern-title"><div><span>紫金规律扫描 · 严格样本外</span><h3>{zijinPatternDiscovery.conclusion.message}</h3><p>候选只在相对高低位或 VWAP 偏离后，三分钟动量/MA5 斜率实时转向时产生；同方向至少间隔 {zijinPatternDiscovery.methodology.candidateCooldownMinutes} 分钟，同一交易日最多 {zijinPatternDiscovery.methodology.maxTradesPerDay} 笔。</p></div><em>{zijinPatternDiscovery.conclusion.deployment}</em></div>
        <div className="zijin-pattern-metrics"><p><span>因果候选场景</span><b>{zijinPatternDiscovery.dataset.labeledScenarios.toLocaleString()}</b><small>未来数据仅作结果标签</small></p><p><span>完整交易日</span><b>{zijinPatternDiscovery.dataset.tradingDays.toLocaleString()}</b><small>{zijinPatternDiscovery.dataset.firstDate}—{zijinPatternDiscovery.dataset.lastDate}</small></p><p><span>正T通过规律</span><b>{zijinPatternDiscovery.acceptedRuleCount.positive}</b><small>训练期 + 2025 双门槛</small></p><p><span>反T通过规律</span><b>{zijinPatternDiscovery.acceptedRuleCount.reverse}</b><small>目标胜率 {(zijinPatternDiscovery.conclusion.winRateTarget*100).toFixed(0)}%</small></p></div>
        <div className="zijin-pattern-next"><b>这不是停止训练，而是淘汰伪规律</b><span>仅凭紫金自身价量，暂未发现能跨年份稳定复现的固定规则。下一轮将补入 {zijinPatternDiscovery.conclusion.nextRequiredFactors.join('、')}，再按 2022–2024 训练、2025 验证、2026 盲测。</span></div>
      </div>
