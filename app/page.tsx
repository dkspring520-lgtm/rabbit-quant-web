...
import { Fragment, useEffect, useMemo, useRef, useState, type CSSProperties, type FormEvent, type KeyboardEvent as ReactKeyboardEvent, type PointerEvent as ReactPointerEvent } from "react";
...
type ZijinStrategyExperiment = "formal-v4" | "closure-first";
...
type MultiDayBacktestResult = BatchMetrics & { code:string; name:string; modeLabel:string; requestedDays:number; testedDays:number; firstDate:string; lastDate:string; noTrade:number; averageNet:number; outcomes:{date:string;trades:number;wins:number;net:number;candidates:number}[] };
...
export default function Home() {
  ...
  const [accountMembership,setAccountMembership]=useState<Membership|null>(null);
  ...
  const zijinAhLinkage=useMemo(()=>{
    ...
    const stale=marketSession.live&&(!Number.isFinite(sourceTime)||Date.now()-sourceTime>120_000);
  ...
  },[stock?.code,zijinHkMarket,minutePoints,activeQuote?.previousClose,marketSession.live]);
  ...
  },[chartModel,zijinAhLinkage,activeQuote?.previousClose]);
    ...
    <main className={`app-shell minimal-ui session-${marketSession.tone}`}>
      ...
      <section className="lower-panel">
        ...
        <div className={`agents ${agentOpen ? 'open' : ''}`}>
          ...
          {agentOpen&&<div className="agent-grid">{liveAgents.map((agent,i)=><button className="agent" key={agent.name} onClick={()=>setActiveView("智能训练")} aria-label={`查看${agent.name}训练详情`}><span className={`agent-icon a${i}`}><img src={agent.avatar} alt={`${agent.name} AI头像`}/></span><span><b>{agent.name}</b><small>{agent.role}</small></span><em><i/>{agent.state}</em><strong>{agent.value}</strong></button>)}</div>}
        </div>
      </section>
...
function AuthView({onAuthenticated,onBack,onDemo,theme,onToggleTheme}:{onAuthenticated:(name:string,isNew:boolean,remember:boolean,membership:Membership|null)=>void;onBack:()=>void;onDemo:()=>void;theme:UiTheme;onToggleTheme:()=>void}) {
...
function SingleStockResearchView({accountName,stock,quote,marketData,profile,profitMode,position,manualCount,onOpenConsole}:{accountName:string;stock:{code:string;name:string;price:string;change:string};quote:MarketData['quote']|undefined;marketData:MarketData|null;profile:string;profitMode:ProfitMode;position:StockPosition;manualCount:number;onOpenConsole:()=>void}) {
  ...
  const {
    ...
    round9Protocol:zijinRound9Protocol,
  }=zijinResearchBundle;
...
function BacktestView({ profile, setProfile, profitMode, setProfitMode, position, stock, stocks, activeStock, onSelectStock }: { profile: StrategyProfile; setProfile: (value: StrategyProfile) => void; profitMode:ProfitMode; setProfitMode:(value:ProfitMode)=>void; position:StockPosition; stock:{code:string;name:string;price:string;change:string}; stocks:{code:string;name:string;price:string;change:string}[]; activeStock:number; onSelectStock:(index:number)=>void }) {
  ...
  const runMultiDay = async () => {
    ...
    setReplayProgress({value:5,detail:`正在获取 ${stock.code} 最近 ${multiDayCount} 个交易日`});
    setRunStatus(`正在读取 ${stock.code} 多日真实分时…`);
    try {
      let fetched=await fetchStock(stock.code,multiDayCount);
      if(stock.code==="601899"){
        const archiveResponse=await fetch(`/api/personal-replay-sessions?code=601899&limit=${multiDayCount+20}`,{cache:"no-store"});
        if(!archiveResponse.ok)throw new Error("Zijin archive unavailable");
        const archivePayload=await archiveResponse.json() as {sessions?:IntradaySession[]};
      ...
      const results=sessions.map((session,index)=>{
        ...
        return {date:session.date,data,result:calculated};
      });
      ...
      setMultiDay(report);
      setRunStatus(`连续 ${results.length} 日完成：${report.completed} 个闭环，扣费后胜率 ${report.completed?(report.wins/report.completed*100).toFixed(1):"0.0"}%`);
