...
type SliceEvent={time:string;side:string;level:number};
type SliceState={levels:Record<string,number>;events:SliceEvent[];[key:string]:unknown};
export function SliceScorePanel({available,buy,sell,time,price}:{available:boolean;buy:number;sell:number;time:string;price:number}) {
  const [state,setState]=useState<SliceState>({levels:{},events:[]});
  // advanceSlice is a state machine: each live snapshot must retain prior
  // threshold events so the same level is not announced repeatedly.
  // eslint-disable-next-line react-hooks/set-state-in-effect
  useEffect(()=>{setState(previous=>advanceSlice(previous,{available,buy,sell,time,price}) as SliceState);},[available,buy,sell,time,price]);
