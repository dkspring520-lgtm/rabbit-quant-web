...
test("V5 hierarchical research is sealed, forward-safe, and cannot alter V4", async () => {
  ...
  assert.equal(protocol.execution.session[0], "09:35");
  assert.equal(protocol.openingDirection.status, "forward-shadow-only");
  assert.equal(protocol.openingDirection.affectsFormalExecution, false);
  assert.equal(protocol.openingDirection.auction.requiresForwardL2, true);
  assert.equal(protocol.openingDirection.auction.usesHistoricalAuctionBook, false);
  assert.equal(protocol.openingDirection.stages["09:30-09:35"].allowsCandidate, false);
  assert.equal(protocol.openingDirection.historicalBarFallback.firstActionAt, "09:35");
  assert.equal(protocol.openingDirection.historicalBarFallback.doesNotUseAuctionOrL2, true);
  ...
  assert.match(runner, /opening_direction_permission/);
  assert.match(runner, /openingDirectionUsesForwardL2Only/);
});
