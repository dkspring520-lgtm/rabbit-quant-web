...
test("intraday crosshair locks to an already observed minute",()=>{
  ...
  assert.match(source,/let nearest=visiblePoints\[0\]/);
...
test("mouse, touch and keyboard can inspect the same causal chart",()=>{
  ...
  assert.match(source,/setIntradayCursorTime\(visiblePoints\[nextIndex\]\.time\)/);
...
test("price and main-force SVGs share minute coordinates and the desktop gutter",()=>{
  assert.match(source,/x=\{viewportChartX\(bar\.time\)-1\.45\}/);
  assert.match(source,/x:viewportSlotX\(slot\)/);
