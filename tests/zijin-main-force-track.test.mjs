...
test("main-force chart is tracking evidence only and collector persists minute fields", async () => {
  ...
  assert.match(page, /buildReplayChartObservations\(stock\?\.code,minutePoints,\(liveEngine\.observations \?\? \[\]\) as ReplayObservation\[\]\)/);
  assert.doesNotMatch(page, /buildReplayChartObservations\([^\n]+zijinRepairHistory\)/);
  assert.match(page, /shadowObservations:isZijinStock/);
  assert.match(page, /\.\.\.zijinRepairHistory/);
